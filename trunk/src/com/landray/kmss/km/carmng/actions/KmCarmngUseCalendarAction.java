 ...
 * 用车日历 Action
 */
public class KmCarmngUseCalendarAction extends ExtendAction {
	...
	private JSONObject genCategoryJSON(List<String> motorcadeSetIds)
		...
		hql.setWhereBlock(whereBlock);
		System.out.println(getCarmngMotorcadeSetServiceImp().findList(HQLInfo).size());*/
	...
	private void getListJSON(KmCarmngDispatchersInfoList main, JSONArray list) throws Exception {
		...
		if (null != appList && appList.size() > 0) {
			for (KmCarmngApplication app : appList) {
				...
				String title = app.getFdApplicationPerson().getFdName();
				if (null != app.getFdApplicationDept()) {
					title += "(" + app.getFdApplicationDept().getFdName() + ")用车";
				} else {
					if (null != app.getFdApplicationPerson().getFdParent()) {
						title += "(" + app.getFdApplicationPerson().getFdParent().getFdName();
					}
				}
