...
<template:include ref="default.simple" spa="true">
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="${ lfn:message('hr-staff:hrStaffPersonExperience.type.training')}"></ui:menu-item>
		</ui:menu>
	</template:replace>
		...
		<div class="lui_list_nav_frame">
			<ui:accordionpanel>
				<c:import url="/hr/staff/import/nav.jsp" charEncoding="UTF-8">
					<c:param name="type" value="training" />
				</c:import>
			</ui:accordionpanel>
		</div>
	</template:replace>
	<template:replace name="body">
		...
		<c:import url="/hr/staff/hr_staff_person_experience/experience_common_criteria.jsp" charEncoding="UTF-8">
			<c:param name="experienceType" value="training" />
		</c:import>
			...
			<div class="lui_list_operation_sort_btn">
				...
				<div class="lui_list_operation_sort_toolbar">
					...
					<list:sortgroup>
					    ...
					    <list:sort property="fdBeginDate" text="${lfn:message('hr-staff:hrStaffPersonExperience.fdBeginDate') }" group="sort.list"></list:sort>
					    <list:sort property="fdEndDate" text="${lfn:message('hr-staff:hrStaffPersonExperience.fdEndDate') }" group="sort.list"></list:sort>
					</list:sortgroup>
					</ui:toolbar>
				</div>
			</div>
				...
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar count="5">
						...
						<kmss:authShow roles="ROLE_HRSTAFF_DELETE">
							<ui:button text="${lfn:message('button.deleteall') }" onclick="_delete('${LUI_ContextPath}/hr/staff/hr_staff_person_experience/training/hrStaffPersonExperienceTraining.do?method=deleteall');" order="3"></ui:button>
						</kmss:authShow>
					</ui:toolbar>
				</div>
			</div>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/hr/staff/hr_staff_person_experience/training/hrStaffPersonExperienceTraining.do?method=list'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/hr/staff/hr_staff_person_info/hrStaffPersonInfo.do?method=view&fdId=!{personInfoId}&anchor=experienceTraining" name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial> 
				<list:col-auto props="nameAccount;fdTrainingName;fdBeginDate;fdEndDate;fdTrainingUnit;fdMemo;operations"></list:col-auto>
			</list:colTable>
		</list:listview> 
		<list:paging></list:paging>
		
		<c:import url="/hr/staff/hr_staff_person_experience/experience_type_script.jsp" charEncoding="UTF-8">
			<c:param name="downloadTempletUrl" value="${LUI_ContextPath}/hr/staff/hr_staff_person_experience/training/hrStaffPersonExperienceTraining.do?method=downloadTemplet" />
			<c:param name="importType" value="training" />
			<c:param name="importTitle" value="${lfn:message('hr-staff:hrStaffPersonExperience.type.training') } - ${lfn:message('hr-staff:hrStaff.import.button.modify') }" />
		</c:import>
	</template:replace>
</template:include>
