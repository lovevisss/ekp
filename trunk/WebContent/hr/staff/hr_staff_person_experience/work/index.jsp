...
<template:include ref="default.simple" spa="true">
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="${ lfn:message('hr-staff:hrStaffPersonExperience.type.work')}"></ui:menu-item>
		</ui:menu>
	</template:replace>
	...
	<template:replace name="body">
		...
		<c:import url="/hr/staff/hr_staff_person_experience/experience_common_criteria.jsp" charEncoding="UTF-8">
			<c:param name="experienceType" value="work" />
		</c:import>
				...
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar count="5">
						...
						<kmss:authShow roles="ROLE_HRSTAFF_DELETE">
							<ui:button text="${lfn:message('button.deleteall') }" onclick="_delete('${LUI_ContextPath}/hr/staff/hr_staff_person_experience/work/hrStaffPersonExperienceWork.do?method=deleteall');" order="3"></ui:button>
						</kmss:authShow>
					</ui:toolbar>
				</div>
			</div>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/hr/staff/hr_staff_person_experience/work/hrStaffPersonExperienceWork.do?method=list'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/hr/staff/hr_staff_person_info/hrStaffPersonInfo.do?method=view&fdId=!{personInfoId}&anchor=experienceWork" name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial> 
				<list:col-auto props="nameAccount;fdCompany;fdPosition;fdBeginDate;fdEndDate;fdDescription;fdReasons;operations"></list:col-auto>
			</list:colTable>
		</list:listview> 
		<list:paging></list:paging>
		
		<c:import url="/hr/staff/hr_staff_person_experience/experience_type_script.jsp" charEncoding="UTF-8">
			<c:param name="downloadTempletUrl" value="${LUI_ContextPath}/hr/staff/hr_staff_person_experience/work/hrStaffPersonExperienceWork.do?method=downloadTemplet" />
			<c:param name="importType" value="work" />
			<c:param name="importTitle" value="${lfn:message('hr-staff:hrStaffPersonExperience.type.work') } - ${lfn:message('hr-staff:hrStaff.import.button.modify') }" />
		</c:import>
	</template:replace>
</template:include>
