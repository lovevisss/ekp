...
<template:include ref="default.simple" spa="true">
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="${ lfn:message('hr-staff:hrStaffPersonExperience.type.education')}"></ui:menu-item>
		</ui:menu>
	</template:replace>
		...
		<div class="lui_list_nav_frame">
			<ui:accordionpanel>
				<c:import url="/hr/staff/import/nav.jsp" charEncoding="UTF-8">
					<c:param name="type" value="education" />
				</c:import>
			</ui:accordionpanel>
		</div>
	</template:replace>
	<template:replace name="body">
		...
		<c:import url="/hr/staff/hr_staff_person_experience/experience_common_criteria.jsp" charEncoding="UTF-8">
			<c:param name="experienceType" value="education" />
		</c:import>
			...
			<div class="lui_list_operation_sort_btn">
				...
				<div class="lui_list_operation_sort_toolbar">
					...
					<list:sortgroup>
					    ...
					    <list:sort property="fdBeginDate" text="${lfn:message('hr-staff:hrStaffPersonExperience.education.fdBeginDate') }" group="sort.list"></list:sort>
					    <list:sort property="fdEndDate" text="${lfn:message('hr-staff:hrStaffPersonExperience.education.fdEndDate') }" group="sort.list"></list:sort>
					</list:sortgroup>
					</ui:toolbar>
				</div>
			</div>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/hr/staff/hr_staff_person_experience/education/hrStaffPersonExperienceEducation.do?method=list'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/hr/staff/hr_staff_person_info/hrStaffPersonInfo.do?method=view&fdId=!{personInfoId}&anchor=experienceEducation" name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial> 
				<list:col-auto props="nameAccount;fdSchoolName;fdMajor;fdEducation;fdDegree;fdBeginDate;fdEndDate;fdMemo;operations"></list:col-auto>
			</list:colTable>
		</list:listview> 
		<list:paging></list:paging>
		
		<c:import url="/hr/staff/hr_staff_person_experience/experience_type_script.jsp" charEncoding="UTF-8">
			<c:param name="downloadTempletUrl" value="${LUI_ContextPath}/hr/staff/hr_staff_person_experience/education/hrStaffPersonExperienceEducation.do?method=downloadTemplet" />
			<c:param name="importType" value="education" />
			<c:param name="importTitle" value="${lfn:message('hr-staff:hrStaffPersonExperience.type.education') } - ${lfn:message('hr-staff:hrStaff.import.button.modify') }" />
		</c:import>
	</template:replace>
</template:include>
