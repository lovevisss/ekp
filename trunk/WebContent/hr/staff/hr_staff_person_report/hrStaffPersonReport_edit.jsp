...
<template:include ref="default.edit" sidebar="auto">
	<template:replace name="title">
		<c:choose>
			<c:when test="${ hrStaffPersonReportForm.method_GET == 'add' }">
				<c:out value="${ lfn:message('button.add') } - ${ lfn:message('hr-staff:table.hrStaffPersonReport') }"></c:out>	
			</c:when>
			<c:otherwise>
	...
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
			<ui:button text="${ lfn:message('hr-staff:hrStaffPersonReport.addAndView') }" 
				onclick="Com_Submit(document.hrStaffPersonReportForm, 'save');" order="1">
			</ui:button>
	...
	<template:replace name="head">
		...
		<style>
			.tb_simple #custom{display:inline-block; vertical-align:middle;}
			.tb_simple #custom .inputselectsgl, .tb_simple #custom .tb_normal .inputselectsgl{vertical-align:middle; position:relative; top:-1px;}
		</style>
	</template:replace>
	<template:replace name="content">
		<html:form action="/hr/staff/hr_staff_person_report/hrStaffPersonReport.do" >
			<p class="lui_form_subject">
				<c:choose>
					<c:when test="${ hrStaffPersonReportForm.method_GET == 'add' }">
						<bean:message key="hr-staff:hrStaffPersonReport.type.${param.fdReportType}"/>
					</c:when>
					<c:otherwise>
			...
			<div class="lui_form_content_frame" style="padding-top:20px">
				...
				<div id="div_condtionArea">
					...
					<div id="div_condtionSection">
						...
						<html:hidden property="fdReportType" value="${param.fdReportType}"/>
						<table class="tb_simple" width="100%">
							...
							<tr>
								...
								<td>
									<xform:address textarea="false" mulSelect="true" propertyId="authReaderIds" propertyName="authReaderNames" style="width:80%;">
									</xform:address>
								</td>
							</tr>
							<%--统计期间 --%>
							<tr>
								...
								<td>
									<xform:select property="fdPeriod" showPleaseSelect="false">
									...
									<span id="custom" style="display: none;">
										<xform:datetime property="fdBeginPeriod" dateTimeType="date"></xform:datetime>
										${lfn:message('hr-staff:hrStaffPersonReport.fdPeriod.to')}
										<xform:datetime property="fdEndPeriod" dateTimeType="date"></xform:datetime>
									</span>
								</td>
							</tr>
							<%-- 年龄区间 --%>
							<c:if test="${'reportAge' eq param.fdReportType}">
							<tr>
								<td class="td_normal_title"><bean:message bundle="hr-staff" key="hrStaffPersonReport.age.fdAgeRange" /></td>
								<td>
									<xform:text property="fdAgeRange" validators="range(3,20)"></xform:text>
									${lfn:message('resource.period.type.year.name')}
								</td>
							</tr>
							</c:if>
							<%-- 司龄区间  --%>
							<c:if test="${'reportWorkTime' eq param.fdReportType}">
							<tr>
								<td class="td_normal_title"><bean:message bundle="hr-staff" key="hrStaffPersonReport.workTime.fdAgeRange" /></td>
								<td>
									<xform:select property="fdAgeRange" validators="required" showPleaseSelect="false">
							...
							<tr>
								...
								<td>
									...
									<br>
									<xform:checkbox property="fdStatus" showStatus="edit" validators="required">
				...
				<div id="div_previewArea">
					<ui:button text="${lfn:message('hr-staff:hrStaffPersonReport.page.resultsPreview')}" onclick="window.stat.statExecutor();"></ui:button>
				</div>
		...
		<script language="JavaScript">
			...
			seajs.use(['lui/jquery'], function($) {
				$(function() {
					$("select[name=fdPeriod]").change(function(){
					...
					$("input[name=fdStatusAll]").click(function() {
						var isChecked = $(this).is(":checked");
						$.each($("input[name=_fdStatus]"), function(i, n) {
							var statusChecked = $(n).is(":checked");
							if(isChecked) {
								if(!statusChecked) {
									$(n).click();
								}
					...
					<c:if test="${ hrStaffPersonReportForm.method_GET == 'add' }">
						// 新增页面，员工状态默认为全选
						$("input[name=fdStatusAll]").click();
					</c:if>
				});
			});
		</script>
