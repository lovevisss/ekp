	...
	<script type="text/javascript">
		...
		<!-- 筛选器 -->
 		<list:criteria id="criteria">
			<list:cri-ref key="_fdKey" ref="criterion.sys.docSubject" title="${ lfn:message('hr-staff:hrStaffPersonInfo.criteria.fdKey') }" style="width:400px;">
			</list:cri-ref>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/hr/staff/hr_staff_person_family/hrStaffPersonFamily.do?method=list'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/hr/staff/hr_staff_person_info/hrStaffPersonInfo.do?method=view&fdId=!{personInfoId}&anchor=experienceFamily" name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial> 
				<list:col-auto props="fdStaffFdName,fdName,fdRelated,fdCompany,fdOccupation,operations"></list:col-auto>
			</list:colTable>
		</list:listview> 
		...
		<script type="text/javascript">
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic) {
				...
				window.edit = function(id) {
					Com_OpenWindow('<c:url value="/hr/staff/hr_staff_person_family/hrStaffPersonFamily.do" />?method=edit&fdId=' + id);
				};
				// 导入
				window._modify = function() {
					var actionUrl = "${LUI_ContextPath}/hr/staff/hr_staff_person_family/hrStaffPersonFamily.do?method=fileUpload";
					var downLoadUrl = "${LUI_ContextPath}/hr/staff/hr_staff_person_family/hrStaffPersonFamily.do?method=downloadTemplet";
					dialog.iframe('/hr/staff/upload_files/common_upload_download.jsp?uploadActionUrl=' + actionUrl + '&isRollBack=false'+'&downLoadUrl='+downLoadUrl, 
							"${ lfn:message('hr-staff:hrStaffPerson.family') }", function(data) {
							topic.publish('list.refresh');
					}, {
						width : 680,
						height : 380
				...
				window._delete = function(id) {
					...
					dialog.confirm('<bean:message key="page.comfirmDelete"/>', function(value) {
						if(value == true) {
							...
							$.ajax({
								url : "${LUI_ContextPath}/hr/staff/hr_staff_person_family/hrStaffPersonFamily.do?method=deleteall",
								type : 'POST',
