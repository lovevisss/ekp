...
$(function () {
  ...
  //随机背景图
  // var bg = templatePath + 'images/' + get_random_bg();
  // var num =Math.round(Math.random()*2)+1;
  var bg = templatePath + "login_bg/bg_login.png";

  $(".login-background-img > #login-bgImg").attr("src", bg);
  $(".login-background-img > img").attr("src", bg);
  var arrIcon = $(".login-switch-icon-item");
  var curIndex = 0;
  var timer = setInterval(removeIconClass, 6500);

  function removeIconClass() {
    arrIcon.each(function (index, el) {
      //animation-lightning
      if ($(el).hasClass("lbi_icon-active")) {
        $(el).removeClass("lbi_icon-active");
        $(el).children().last().removeClass("animation-lightning");
        curIndex = index;
        return;
      }
    });
  ...
  function switchIcon() {
    ...
    el = $(arrIcon[curIndex]);
    el.addClass("lbi_icon-active");
    el.children().last().addClass("animation-lightning");
    timer = setInterval(removeIconClass, 6500);
  }
  //二维码
  $(".login-dropbox").hover(
    function () {
      ...
      $(this)
        .find(".login-dropbox-toggle > .iconbox")
        .stop()
        .animate(
          {
            left: "-45px",
        ...
        .end()
        .find(".login-dropbox-menu")
        .css("height", iframe_height)
        .stop(true, true)
        .show(300);
    },
    function () {
...
function dropdown(obj) {
  ...
  if (ul.length > 0) {
    ...
    $(ul)
      .attr("id", "dropdownOptionsFor" + className)
