...
define(function(require, exports, module) {
	...
	var ResAbstractCalenderMode = Calendar.AbstractCalenderMode.extend({
		resAbstractConfig : null,
		initialize : function($super,_config){
			this.resAbstractConfig = _config;
			$super(_config);
			this.pageno = 1;
		...
		_render:function(start ,end ,datas,renderType){
			if(!renderType){
				...
				//防止重复渲染-绑定错误calendar对象，重新绑定 #164585 update by zhouw 20220519
				this.calendarDiv = $(this.resAbstractConfig.container.selector) || $(".lui_calendar_content").eq(0);
		...
		fortmatTitle : function(data){
			...
			if(data.allDay){
				...
				if(data.kmCarmngUseCalendar){
					title=title+" ~ "+$.fullCalendar.formatDate(data['end'],"yyyy-MM-dd HH:mm");
				}
			}
			title+=" "+data.title;
		...
		formatTitleAttr : function(data){
			if(data.kmCarmngUseCalendar){
				return $.fullCalendar.formatDate(data['start'],"yyyy-MM-dd HH:mm")+" ~ "+$.fullCalendar.formatDate(data['end'],"yyyy-MM-dd HH:mm")+" "+data.title;
			}
			return data.title;
		},
