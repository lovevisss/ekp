...
<list:data>
	<list:data-columns list="${queryPage.list }" var="sysAttendSynDing"
		...
		<list:data-column col="fdUserAddress" title="${lfn:message('sys-attend:sysAttendMain.export.fdLocation') }">
			...
			<c:if test="${not empty sysAttendSynDing.fdWifiName }">
				<c:choose>
					<c:when test="${not empty sysAttendSynDing.fdUserAddress }">
						(<c:out value="${sysAttendSynDing.fdWifiName }"></c:out> )
					</c:when>
					<c:otherwise>
						<c:out value="${sysAttendSynDing.fdWifiName }"></c:out>
					</c:otherwise>
		...
		<list:data-column col="fdTimeResult" title="${lfn:message('sys-attend:sysAttendMain.export.fdStatus') }">
			<c:choose>
				...
				<c:otherwise>
					<c:choose>
						<c:when test="${sysAttendSynDing.fdTimeResult=='Late' || sysAttendSynDing.fdTimeResult=='SeriousLate'  || sysAttendSynDing.fdTimeResult=='Absenteeism' }">
							${lfn:message('sys-attend:sysAttendMain.fdStatus.late') }
						</c:when>
