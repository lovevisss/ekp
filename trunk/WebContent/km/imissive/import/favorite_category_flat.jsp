...
seajs.use(['lui/jquery','lui/util/str','lui/dialog'], function($,str,dialog) {
	...
	window.openCategory = function(){
		if("${JsParam.isSimpleCategory}" == 'false' || '${JsParam.cateType}' == 'globalCategory'){
			dialog.categoryForNewFile('${JsParam.modelName}','${addUrl}',false,null,null,null,null,null,true);
		}else{
	...
	<div id="favouriteCateDiv">
		<ui:dataview id="favouriteCate">
			<ui:source type="AjaxJson">
				{url: '/sys/person/sys_person_favorite_category/sysPersonFavoriteCategory.do?method=favoriteWithCate&modelName=${JsParam.modelName}'}
			</ui:source>
			...
			<c:choose>
							...
							<li class="zj-cate-item">
								<div class="link-box-heading">
									<div class="zj-cate-title">
										<div class="zj-cate-c"><span>{%env.fn.formatText(data[i].text)%}</span></div>
									</div>
									<div class="link-footer">
										...
										<a class="btn-add"  href="javascript:void(0)" onclick="javascript:openCreate('{%data[i].value%}');" title="{%env.fn.formatText(data[i].text)%}"><bean:message key="button.add"/></a>
									</div>
								</div>
							</li>
						$}
			 ...
			 {$
			 		<li class="zj-cate-item zj-cate-all" onclick="openCategory();">
