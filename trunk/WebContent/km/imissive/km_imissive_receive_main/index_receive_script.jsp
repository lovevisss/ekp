...
seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
	...
	window.expanded = function(){
		...
		LUI('receiveCriteria').expandCriterions(!LUI('receiveCriteria').expand);
		
	};
	
	window.doSearchAction = function(evt){
		var topicEvent = {
				criterions :  LUI("receiveCriteria")._buildCriteriaSelectedValues(),
				query : []
			};
		var obj = {};
		var values = [];
        ...
        var j_path_value;
        if("${param.contentId}" == "kmImissiveReceivePContent"){
            j_path_value = "/listAllPublish/receive";
        }
        if("${param.contentId}" == "kmImissiveReceiveCContent"){
            j_path_value = "/listCreate/receive";
        }
        if("${param.contentId}" == "kmImissiveReceiveFContent"){
            j_path_value = "/filing/receive";
        }
        if("${param.contentId}" == "kmImissiveReceiveDContent"){
            j_path_value = "/discard/receive";
        }
        if("${param.contentId}" == "kmImissiveReceiveContent"){
            j_path_value = "/listAll/receive";
        }
        if(j_path_value != ""){
            var obj2 = {};
            var values2 = [];
	...
	window.addOffLineDoc = function() {
		var url = "/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=addOffLine&fdTemplateId=!{id}";
		dialog.categoryForNewFile('com.landray.kmss.km.imissive.model.KmImissiveReceiveTemplate',
				url, false, null, null, cateId, null, null, true); 
	};
	
	//新建
	window.addDoc = function() {
		/* var url = '${LUI_ContextPath}/km/imissive/km_imissive_receive_main/createDoc_receive.jsp';
		url = Com_SetUrlParameter(url, 'mainModelName', 'com.landray.kmss.km.imissive.model.KmImissiveReceiveMain');
		url = Com_SetUrlParameter(url, 'modelName', 'com.landray.kmss.km.imissive.model.KmImissiveReceiveTemplate');
		...
		LUI.pageOpen(url, '_rIframe'); */
		 ...
		 dialog.categoryForNewFile(
				...
				'/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=add&fdTemplateId=!{id}',false,null,null,cateId,null,null,true); 
	};
	window.openSupervise = function(){
		...
		var url  = '${KMSS_Parameter_ContextPath}km/supervise/km_supervise_main/kmSuperviseMain.do?method=add&modelName=com.landray.kmss.km.imissive.model.KmImissiveReceiveMain&modelId='+values[0];
	...
	//批量打印
		...
		var url  = '<c:url value="/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=printBatch"/>'+'&fdIds='+values;
		Com_OpenWindow(url);
	};
	//删除
	window.delDoc = function(){
		...
		/* dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
			if(value==true){
				...
				$.post('<c:url value="/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=deleteall"/>'+'&categoryId='+cateId+'&nodeType='+nodeType,
						$.param({"List_Selected":values},true),delCallback,'json');
			}
		}); */
		var url='<c:url value="/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=deleteall"/>'+'&categoryId='+cateId+'&nodeType='+nodeType;
		if(docStatus == '10'){
			var url='<c:url value="/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=deleteall&draft=true"/>'+'&categoryId='+cateId+'&nodeType='+nodeType;
		}
		var config = {
			url : url, // 删除数据的URL
	...
	window.showButtons = function(cateId,nodeType){
		 if(AuthCache[cateId]){
          ...
          window.showFilingBtn = function(cateId,nodeType){
    			 var checkDelUrl ="/sys/archives/sys_archives_file_template/sysArchivesFileTemplate.do?method=fileDocAll&modelName=com.landray.kmss.km.imissive.model.KmImissiveReceiveMain&categoryId="+cateId+"&nodeType="+nodeType;
		 var data = new Array();
