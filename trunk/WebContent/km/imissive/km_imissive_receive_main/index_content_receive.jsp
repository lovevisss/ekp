...
<%@ page import="com.landray.kmss.sys.organization.model.SysOrgElement" %>
 <template:include ref="default.simple" spa="true">
  ...
  <list:criteria id="receiveCriteria"  cfg-canExpand="false">
	  	<c:if test="${empty JsParam.filingBox}">
		  <list:tab-criterion title="" key="docStatus">
			  <list:box-select>
				  <list:item-select type="lui/criteria/select_panel!TabCriterionSelectDatas" cfg-enable="false" cfg-if="param['mydoc']!='approval' && param['mydoc']!='approvalReceive'&& param['docStatus']!='00'&& param['fdIsFiling']!='1'&&param['flag']!='finished'&& param['fdIsOffLine']!='1'">
		...
		<%-- <list:tab-criterion title="" key="docStatus">
			<list:box-select>
				<list:item-select type="lui/criteria/select_panel!TabCriterionSelectDatas" cfg-enable="false" cfg-if="(param['mydoc']=='approval'||param['mydoc']=='approvalReceive')&& param['docStatus'] !='00'&& param['fdIsFiling']!='1'">
					<ui:source type="Static">
						[{text:'${ lfn:message('status.examine')}',value:'20'},
	  ...
	  <kmss:authShow roles="ROLE_KMIMISSIVE_VIEWMYDEPTDOC_RECEIVE">
		  <%
	  ...
	  <list:cri-ref ref="criterion.sys.category" key="fdTemplate" multi="false" title="${ lfn:message('km-imissive:kmImissiveReceiveMain.criteria.fdTemplate') }" expand="false">
		  <list:varParams modelName="com.landray.kmss.km.imissive.model.KmImissiveReceiveTemplate"/>
		</list:cri-ref>
		...
		<list:cri-auto modelName="com.landray.kmss.km.imissive.model.KmImissiveReceiveMain" property="fdReceiveTime;fdSigner;fdSignTime;fdIsReturn;docCreateTime"/>
  		<list:cri-criterion expand="false" title="${lfn:message('km-imissive:kmImissiveReceiveMain.fdDeadTime') }" key="fdDeadType" multi="false">
             <list:box-select>
	...
	<div class="lui_list_operation">
		<div class="lui_list_operation_sort_btn">
			<div class="lui_list_operation_sort_toolbar">
				<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" id="sort_toolbar_receive">
					<list:sortgroup>
						<list:sort property="docCreateTime" text="${ lfn:message('km-imissive:kmImissiveReceiveMain.docCreateTime')}" group="sort.list"></list:sort>
						...
						<list:sort property="fdDeadTime" text="${lfn:message('km-imissive:kmImissiveReceiveMain.fdDeadTime')}" group="sort.list"></list:sort>
					</list:sortgroup>
				</ui:toolbar>
			</div>
		</div>
			...
			<div style="display: inline-block;vertical-align: middle;">
				<ui:toolbar count="8" id="Btntoolbar">
					...
					<kmss:authShow roles="ROLE_KMIMISSIVE_RECEIVE_CREATE">
						...
						<ui:button text="${lfn:message('km-imissive:button.addOffLine.receive')}" onclick="addOffLineDoc()" order="1"
										cfg-if="param['docStatus'] != '00' && param['docStatus'] != '32' && param['fdIsFiling'] != '1' && param['fdIsOffLine'] == '1'"></ui:button>	
					</kmss:authShow>
					...
					<ui:button text="${lfn:message('button.export')}" id="export" onclick="listExport('${LUI_ContextPath}/sys/transport/sys_transport_export/SysTransportExport.do?method=listExport&fdModelName=com.landray.kmss.km.imissive.model.KmImissiveReceiveMain')" order="2" ></ui:button>
					</kmss:authShow>
	...
	<ui:fixed elem=".lui_list_operation"></ui:fixed>
			<c:if test="${param.contentId eq 'kmImissiveReceivePContent'}">
				<c:set var="paramStr" value="&q.flag=finished"></c:set>
			</c:if>
			<c:if test="${param.contentId eq 'kmImissiveReceiveCContent'}">
				<c:set var="paramStr" value="&q.mydoc=create"></c:set>
			</c:if>
			<c:if test="${param.contentId eq 'kmImissiveReceiveFContent'}">
				<c:set var="paramStr" value="&q.fdIsFiling=1"></c:set>
			</c:if>
			<c:if test="${param.contentId eq 'kmImissiveReceiveDContent'}">
				<c:set var="paramStr" value="&q.docStatus=00"></c:set>
			</c:if>
			<c:if test="${param.contentId eq 'kmImissiveReceiveOContent'}">
				<c:set var="paramStr" value="&q.fdIsOffLine=1"></c:set>
			</c:if>
	<list:listview id="listview_receive"   >
		<ui:source type="AjaxJson">
				{url:'/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=listChildren&categoryId=${JsParam.categoryId}${paramStr}'}
		</ui:source>
 	...
 	<%@ include file="/km/imissive/km_imissive_receive_main/index_receive_script.jsp"%>
 </template:replace>
 </template:include>	 
