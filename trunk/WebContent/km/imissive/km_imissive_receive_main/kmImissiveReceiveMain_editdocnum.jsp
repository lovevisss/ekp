...
<script>
Com_SetWindowTitle('<bean:message bundle="km-imissive" key="kmImissiveReceiveMain.editdocnum"/>');
function submitForm(){
	var validateFlag = true;
	var fdDocNum = document.getElementsByName("fdReceiveNum")[0];
	if(fdDocNum.value == null || fdDocNum.value == ""){
		alert('<bean:message key="kmImissiveReceiveMain.message.error.fdDocNum" bundle="km-imissive" />');
		validateFlag = false;
	}
	
	if(validateFlag){
		Com_Submit(document.kmImissiveReceiveMainForm, 'updateDocNum');
	}
}

var count = 0;
$(document).ready(function(){
	var fdNoRule = "${kmImissiveReceiveMainForm.fdNoRule}";
	if(fdNoRule!=""){
		/*
		...
		for(var i = 0;i<ruleArr.length;i++){
			if(ruleArr[i]!=""){
				...
				if(ruleArr[i]!='flow'){
					...
					var row = '<tr><td colspan="2"><input type="text" class="inputsgl" style="width:90%" name="flow" onchange="changeNum();" value="${kmImissiveReceiveMainForm.fdFlowNo}"/></td></tr>';
				}
				$('#xxx').append(row);
			}
		}
		...
		for(var i = 0;i<ruleArr.length;i++){
			if(ruleArr[i]!=""){
				...
				if(ruleArr[i]!='flow'){
					 ...
					 row += '<div style="float:left;margin-left:15px;width:150px"><input type="text" class="inputsgl" style="width:100%" name="flow" onchange="changeNum();" value="${kmImissiveReceiveMainForm.fdFlowNo}"/></div>';
				}
			}
		}
		$('#numberEle').html(row);
...
function changeNum(){
	...
	document.getElementsByName("fdReceiveNum")[0].value = fdDocNum;
...
<p class="txttitle"><bean:message bundle="km-imissive" key="kmImissiveReceiveMain.editdocnum"/></p>
	...
	<table class="tb_normal" width=95% id="xxx">
		...
		<tr>
			<td colspan="2">
			   ...
			   <font><bean:message  bundle="km-imissive" key="kmImissiveReceiveMain.editdocnum.info"/></font>
			</td>
		</tr>
		<tr>
			<td class="td_normal_title" width=15%>
				...
				<html:text property="fdReceiveNum" style="width:85%" readonly="true"/>
