	...
	<ui:tabpanel id="tabpanelDiv" layout="sys.ui.tabpanel.view" var-count="5" var-average = 'false'>
		...
		<c:if test="${xinChuangWps ne 'true'}">
		  <ui:content  title="${ lfn:message('km-imissive:kmImissiveReceiveMain.attContent') }" id="attPreview" titleicon="lui-tab-icon tab-icon-05">
		 	...
		 	<c:if test="${kmImissiveReceiveMainForm.docStatus == '30' || kmImissiveReceiveMainForm.docStatus == '00'|| kmImissiveReceiveMainForm.docStatus == '10'}">
...
$(document).ready(function(){
	...
	if($('#attachment_content') && "${_isWpsCloudEnable}" != "true"){
		$('#attachment_content').height( (contentH)+"px");
	}
	if($('#attachment_content') && "${_isWpsCenterEnable}" != "true"){
		$('#attachment_content').height( (contentH)+"px");
	}
	if($('#previewAttFrame')){
...
function setJgHeight(){
	if("${kmImissiveReceiveMainForm.fdNeedContent}" == '1'){
		var obj1 = document.getElementById("JGWebOffice_editonline");
		...
		var obj2 = document.getElementById("JGWebOffice_mainonline");
		if(obj2&&Attachment_ObjectInfo['mainonline']&&jg_attachmentObject_mainonline.hasLoad&&jg_attachmentObject_mainonline.ocxObj){
	  	   	LUI("tabpanelDiv").setSelectedIndex(0);
	  	   	if(jgLoadInterval){
	  	   		clearInterval(jgLoadInterval);
	  	   	}
...
%>
	...
	<c:if test="${not empty kmImissiveReceiveMainForm.sysRelationMainForm.sysRelationEntryFormList}">
 	...
 	<c:choose>	
		<c:when test="${kmImissiveReceiveMainForm.docStatus != '30' && kmImissiveReceiveMainForm.docStatus != '00'}">
			<ui:tabpanel id="barRightPanel" channel="barRightPanel" layout="${layout}">
				...
				<c:if test="${existOpinion}">
					<ui:content  title="${ lfn:message('km-imissive:kmImissiveReceiveMain.cirList') }" titleicon="lui-fm-icon lui-fm-icon-4" id="circulation">
				...
				<ui:content  title="${ lfn:message('km-imissive:kmImissiveReceiveMain.attList') }" id="attPreviewList" titleicon="lui-fm-icon lui-fm-icon-5">
