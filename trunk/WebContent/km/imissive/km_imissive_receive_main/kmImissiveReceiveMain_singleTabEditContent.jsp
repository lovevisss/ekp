...
<template:replace name="toolbar">
  ...
  <ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="6"> 
	...
	<c:if test="${kmImissiveReceiveMainForm.method_GET=='edit'}">
		...
		<c:if test="${kmImissiveReceiveMainForm.docStatus=='20'}">
		    <ui:button text="${lfn:message('button.submit') }" order="2" styleClass="lui_widget_btn_primary" isForcedAddClass="true" onclick="submitOrUpdateDoc(document.kmImissiveReceiveMainForm, 'update');">
			</ui:button>
		</c:if>
...
<script type="text/javascript">
	...
	window.onload=function(){
		if(LUI("tabpanelDiv")){
			if('true' == '${receiveContentFlag}' && '0' == '${kmImissiveReceiveMainForm.fdNeedContent}'){
 ...
 <ui:tabpanel id="tabpanelDiv" layout="sys.ui.tabpanel.view" var-count="4" var-average = 'false'>
 	<ui:content id="kmImissiveReceiveMain_baseinfo" titleicon="lui-tab-icon tab-icon-02"  title="${ lfn:message('km-imissive:kmImissiveReceiveMain.baseinfo') }" expand="${expandBaseInfo}">
	 ...
	 <c:if test="${xinChuangWps ne 'true'}">
	 	 <ui:content  title="${ lfn:message('km-imissive:kmImissiveReceiveMain.attContent') }" id="attPreview" titleicon="lui-tab-icon tab-icon-05">
	 	  ...
	 	  <c:import url="/sys/workflow/import/sysWfProcess_edit.jsp" charEncoding="UTF-8">
			<c:param name="formName" value="kmImissiveReceiveMainForm" />
			<c:param name="fdKey" value="reveiveMainDoc" />
...
<template:replace name="barRight">
	...
	<ui:tabpanel id="barRightPanel" channel="barRightPanel" layout="${layout}">
		...
		<ui:content  title="${ lfn:message('km-imissive:kmImissiveReceiveMain.attList') }" id="attPreviewList" titleicon="lui-fm-icon lui-fm-icon-2">
