...
<%
	...
	if(ImissiveUtil.isAcrossYearNumber(fdTemplateId, "com.landray.kmss.km.imissive.model.KmImissiveReceiveTemplate")){
		showYear = "true";
	}
	pageContext.setAttribute("showYear", showYear);
%>	
<c:set var="definedId" value="${param.fdNumberId}" scope="request"/>
<c:set var="initNumberId" value="${not empty param.fdBufferNumId?param.fdBufferNumId:param.fdNumberId}" scope="request"/>
<link rel="stylesheet" href="${LUI_ContextPath}/km/imissive/resource/css/chosen.css?s_cache=${MUI_Cache}" type="text/css"/>
			...
			<center>
				 <table class="tb_normal " width=95% style="margin-top:25px" >
				   <tr>
					...
					<tr>
					    <td class="titleTd"><bean:message bundle="km-imissive" key="kmImissiveSendMain.fdReceiveNum"/></td>
						<td class="titleTd">
	...
	function getUnUseNum(){
		...
		var url="${KMSS_Parameter_ContextPath}km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=getUnUseNum"; 
		 $.ajax({     
   	     type:"post",    
   	     url:url,   
