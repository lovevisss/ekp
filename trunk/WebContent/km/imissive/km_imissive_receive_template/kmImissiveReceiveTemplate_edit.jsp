...
$(document).ready(function(){
	...
	if(method == "add"){
		if($('input:radio[name="sysFormTemplateForms.receiveMainDoc.fdMode"]')){
		...
		if($('select[name="sysFormTemplateForms.receiveMainDoc.fdMode"]')){
			$('select[name="sysFormTemplateForms.receiveMainDoc.fdMode"]').children(":first").remove();
			$('select[name="sysFormTemplateForms.receiveMainDoc.fdMode"]').children(":nth-child(1)").prop("selected","selected");
		}
		...
		if(_isWpsCloudEnable == "true"){
			...
			setTimeout(function(){
				if(!jg_attachmentObject_editonline.getOcxObj()){
					dialog.alert("当前浏览器检测不到金格控件,在线编辑功能无法使用，若是刚安装完金格控件，请刷新页面或重启浏览器试试！");
				}else{
					 if(obj&&Attachment_ObjectInfo['editonline']&&!jg_attachmentObject_editonline.hasLoad){
						jg_attachmentObject_editonline.load();
						jg_attachmentObject_editonline.show();
...
function submitForm(method) {
	if(typeof XForm_BeforeSubmitForm != 'undefined' && XForm_BeforeSubmitForm instanceof Function){
		XForm_BeforeSubmitForm(function(){
			Com_Submit(document.kmImissiveReceiveTemplateForm, method);
		});
	}else{
		Com_Submit(document.kmImissiveReceiveTemplateForm, method);
    }
...
function openBookmarkHelp(){
	datas = [];
	var baseObjs;
	var fdType = $('select[name="sysFormTemplateForms.receiveMainDoc.fdMode"]').find("option:selected").val();
	if(fdType == '2'){
		var fdTempId = document.getElementsByName("sysFormTemplateForms.receiveMainDoc.fdCommonTemplateId")[0].value;
		baseObjs = _XForm_GetCommonExtDictObj(fdTempId);
	}else{
		var customIframe = window.frames['IFrame_FormTemplate_receiveMainDoc'].contentWindow;
		if(!customIframe){
			customIframe = window.frames['IFrame_FormTemplate_receiveMainDoc'];
		}
		if (customIframe.Designer && customIframe.Designer.instance && customIframe.Designer.instance.hasInitialized) {
			...
			var xformTemplateId = document.getElementsByName("sysFormTemplateForms.receiveMainDoc.fdCommonTemplateId")[0].value;
	...
	Com_OpenWindow(Com_Parameter.ContextPath+'km/imissive/km_imissive_receive_main/bookMarks.jsp','_blank'); 
}
</script>
				...
				<table class="tb_normal" width=100%>
					...
					<tr>
						...
						<td width=85% colspan="3">
							...
							%>
							<xform:select property="fdFlowId"  htmlElementProperties="id='numberSelect'" style="width:200px">
					...
					<td width=35%>
						...
						<label class="weui_switch acrossYearNumber">
							<span class="weui_switch_bd">
								<input type="checkbox" ${'true' eq kmImissiveReceiveTemplateForm.fdAcrossYearNumber ? 'checked' : '' } />
								<span></span>
								<small></small>
							</span>
						...
						<script type="text/javascript">
							...
							setYearText(${kmImissiveReceiveTemplateForm.fdAcrossYearNumber});
						</script>
					</td>
					<td class="td_normal_title" width=15%>
					...
					<td width=35%>
						...
						<label class="weui_switch available">
							<span class="weui_switch_bd">
								<input type="checkbox" ${'true' eq kmImissiveReceiveTemplateForm.fdIsAvailable ? 'checked' : '' } />
								<span></span>
								<small></small>
							</span>
						...
						<script type="text/javascript">
							...
							setText(${kmImissiveReceiveTemplateForm.fdIsAvailable});
						</script>
					</td>
				</tr>
				<%-- 所属场所 --%>
							...
							<c:choose>
								<c:when test="${'true' eq xinChuangWps}">
									<jsp:include page="/km/imissive/wps/oaassist/sysAttMain_edit.jsp">
										...
										<jsp:param name="fdModelId" value="${kmImissiveReceiveTemplateForm.fdId}"/>
										<jsp:param name="fdModelName" value="com.landray.kmss.km.imissive.model.KmImissiveReceiveTemplate"/>
										<jsp:param name="formBeanName" value="kmImissiveReceiveTemplateForm"/>
								...
								<c:otherwise>
									...
									<div<c:if test="${empty kmImissiveReceiveTemplateForm.attachmentForms['editonline'].attachments}">style="display:none"</c:if>>
									...
									<div <c:if test="${not empty kmImissiveReceiveTemplateForm.attachmentForms['editonline'].attachments}">style="display:none"</c:if> >
