...
<% if(ISysAuthConstant.IS_AREA_ENABLED) { %> 
	<input type="hidden" name="authAreaId" value="${kmImissiveSignMainForm.authAreaId}">
<% } %>
<input type="hidden" name="fdTemplateId" value="${kmImissiveSignMainForm.fdTemplateId}"/>
<div id="_docSubject" xform_type="xtext">
<xform:xtext property="docSubject" mobile="${param.mobile eq 'true'? 'true':'false'}" validators="maxLength(500) senWordsValidator(kmImissiveSignMainForm)" required="<%=required%>" style="<%=parse.getStyle()%>"/>
</div>	