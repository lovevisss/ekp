...
<%@page import="com.landray.kmss.km.imissive.actions.KmImissiveRegDetailOpinionAction"%>
...
Com_AddEventListener(window,'load',function() {
	$("#auditNoteTable_${formBeanName}").treeTable({
...
<c:set var="fdMainId" value="${param.fdOpinionId}" scope="request"/>
<%
	new KmImissiveRegDetailOpinionAction().listNote(null, null, request, response);
%>
<%
	LbpmSetting lbpmSetting = new LbpmSetting();
	pageContext.setAttribute("printLbpmPostscript",lbpmSetting.getPrintLbpmPostscript());
%>
<c:set var="mainForm" value="${requestScope[formBeanName]}" scope="request" /> 
<c:if test="${auditNotes!=null && fn:length(auditNotes)>0}">
	 <center>
		<div>
			 <b>意见详情</b>
		 </div>
	...
	<table class="tb_normal" width="100%" id="auditNoteTable_${formBeanName}" >
		<tr class="tr_normal">
		...
		<c:forEach items="${auditNotes}" var="lbpmAuditNote" varStatus="vStatus">
			<c:if test="${lbpmAuditNote.fdActionKey ne '_pocess_end'}">
				<tr
					...
					<td>
						<span title='<c:out value="${lbpmAuditNote.detailHandlerName}" />'>
							<c:out value="${lbpmAuditNote.fdHandler.fdName}" />
						</span>
					</td>
					<td>
