	...
	<%-- 网页签章(首先判断是否启用了网页签章) --%>
	  ...
	  <c:choose>
		  ...
		  <c:when test="${not empty kmImissiveSignMainForm.attachmentForms['dianjuSign'].attachments}">
			  <c:set var="fdSignKey" value="dianjuSign" />
		  </c:when>
			<c:otherwise>
		  ...
		  <%if("content".equals(KmImissiveConfigUtil.getDiplayOrder())){ %>
			  <c:if test="${kmImissiveSignMainForm.fdNeedContent=='1'}">
				  <ui:content  title="${lfn:message('km-imissive:kmImissiveSignMain.docContent')}" id="attContent"  titleicon="lui-tab-icon tab-icon-10">
		  ...
		  <ui:content  title="${ lfn:message('km-imissive:kmImissiveSignMain.attContent') }" id="attPreview" titleicon="lui-tab-icon tab-icon-05">
 	 	 ...
 	 	 <kmss:auth requestURL="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=readViewLog&fdId=${param.fdId}" requestMethod="GET">
		 	 ...
		 	 <c:if test="${kmImissiveSignMainForm.docStatus == '30' || kmImissiveSignMainForm.docStatus == '00'|| kmImissiveSignMainForm.docStatus == '10'}">
		  	     ...
		  	     if(LUI("tabpanelDiv")){
					...
					if(selectedContent.id == "attPreview"){
			  	    ...
			  	    if(selectedContent.id == "attPreview" || selectedContent.id == "attContent"){
						...
						if("${_isWpsCenterEnable}" == 'true' && $('#office-iframe')){
							$(LUI("tabpanelDiv").element).find('[data-lui-mark="panel.contents"]').css('overflow','auto');
						}
			  	    }else{
			  	  		 $(LUI("tabpanelDiv").element).find('[data-lui-mark="panel.contents"]').css('overflow','auto');
...
function setJgHeight(){
	if("${kmImissiveSignMainForm.fdNeedContent}" == '1'){
		var obj1 = document.getElementById("JGWebOffice_editonline");
		if(obj1&&Attachment_ObjectInfo['editonline']&&jg_attachmentObject_editonline.hasLoad&&jg_attachmentObject_editonline.ocxObj){
		...
		if(obj1&&Attachment_ObjectInfo['mainonline']&&jg_attachmentObject_mainonline.hasLoad&&jg_attachmentObject_mainonline.ocxObj){
	  	   	LUI("tabpanelDiv").setSelectedIndex(0);
	  	   	if(jgLoadInterval){
	  	   		clearInterval(jgLoadInterval);
	  	   	}
				...
				if(LUI("tabpanelDiv")){
					...
					for(var j = 0;j< contents.length;j++){
						if(contents[j].id == "attContent"){
							...
							if("${_isWpsCenterEnable}" == "true" && !wpsCenterFlag){
...
%>
	...
	<c:choose>	<%--sys.ui.tabpanel.vertical.icon--%>
		<c:when test="${kmImissiveSignMainForm.docStatus != '30' && kmImissiveSignMainForm.docStatus != '00'}">
			<ui:tabpanel id="barRightPanel" channel="barRightPanel" layout="${layout}">
				<kmss:auth requestURL="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=readViewLog&fdId=${kmImissiveSignMainForm.fdId}" requestMethod="GET">
				...
				<c:if test="${existOpinion}">
					<ui:content  title="传阅意见" titleicon="lui-fm-icon lui-fm-icon-4" id="circulation">
				...
				<ui:content  title="附件列表" id="attPreviewList" titleicon="lui-fm-icon lui-fm-icon-5">
		 	   ...
		 	   <c:if test="${showRelation eq 'true' }">
			 	     <c:import url="/km/imissive/import/sysRelationMain_view.jsp" charEncoding="UTF-8">
						...
						<c:param name="formName" value="kmImissiveSignMainForm" />
						<c:param name="titleicon" value="lui-fm-icon lui-fm-icon-4" />
					</c:import>
				</c:if>
				<ui:event event="indexChanged" args="data" >
			...
			<ui:accordionpanel>
				...
				<c:if test="${showRelation eq 'true' }">
					<c:import url="/km/imissive/import/sysRelationMain_view.jsp" charEncoding="UTF-8">
						<c:param name="layout" value="sys.ui.accordionpanel.simpletitle"/>
						<c:param name="formName" value="kmImissiveSignMainForm" />
						<c:param name="showContent" value="false" />
					</c:import>
				</c:if>
			</ui:accordionpanel>
			<script>
