...
<link type="text/css" rel="stylesheet" href="${LUI_ContextPath}/sys/profile/resource/css/operations.css?s_cache=${LUI_Cache}"/>
	...
	<list:listview id="listview_sign"   >
		<ui:source type="AjaxJson">
				{url:'/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=listChildren&forwardStr=draftlist&q.docStatus=10&q.j_path=${JsParam.j_path}'}
		</ui:source>
 	...
 	<script type="text/javascript">
		...
		seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
			...
			window.delDoc = function(){
				...
				/* 软删除配置 */
				var url='<c:url value="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=deleteall&draft=true"/>';
				var config = {
					url : url, // 删除数据的URL
			...
			window.deleteDoc = function(fdId){
				...
				var config = {
					url : url, // 删除数据的URL
					data : $.param({"List_Selected":values},true), // 要删除的数据
					modelName : "com.landray.kmss.km.imissive.model.KmImissiveSignMain" // 主要是判断此文档是否有部署软删除
				};
				// 通用删除方法
				Com_Delete(config, delCallback);
			};
