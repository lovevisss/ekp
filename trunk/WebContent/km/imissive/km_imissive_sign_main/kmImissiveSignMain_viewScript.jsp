...
<script>
    ...
    Com_IncludeFile("dialog.js|jquery.js|docutil.js");
...
var subject = "${kmImissiveSignMainForm.docSubjectBak}";
...
//#105274 流程分发页面调整
    ...
    function convertFile(fileType){
        ...
        $.ajax({
            url : url,
            type : 'post',
            data : {"fdMainId":"${param.fdId}","converterKey":fileType,"modelKey":"com.landray.kmss.km.imissive.model.KmImissiveSignMain","fdKey":"${contentKey}"},
            dataType : 'text',
            async : false,
            dataType:"json",
            error : function(){
    ...
    window.ofdSursen=function(key){
        ...
        $.ajax({
            url : url,
            type : 'post',
            data : {"fdId":"${param.fdId}","contentKey":key,"modelName":"com.landray.kmss.km.imissive.model.kmImissiveSignMain","convertId":convertId},
            async : false,
            dataType:"json",
            error : function(){
                dialog.alert('请求出错');
                ...
                if(data){
                    ...
                    if(result == 'true'){
                        ...
                        var openUrl ="${LUI_ContextPath}/km/imissive/km_imissive_sign/sursenSign/kmImissiveMain_sursenSign.jsp?fdId=${param.fdId}"  +
                            ...
                            "&fdModelName=com.landray.kmss.km.imissive.model.kmImissiveSignMain";
								...
								success : function(data) {
									if(data == "false"){
										$.ajax({
											...
											success:function(data){
												if(data == "true"){
													dialog.alert("当前签报已经发送过易企签签署，整个签署事件未完成，请等待完成后在发起签署！！");
												}else{
												    ...
												    if(obj_){
														if(Attachment_ObjectInfo[key]){
															 if(Attachment_ObjectInfo[key].ocxObj){
																 ...
																 if(rFlag){
																	 var infoUrl = "/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=openYqqExtendInfo&signId=${param.fdId}";
    ...
    function buildEqbInfo(signType) {
        ...
        signData['applyId'] = '${kmImissiveSignMainForm.fdId}';
        // contractNo由后台生成UUID设值
        signData['contractNo'] = '';
        // signData['contractName'] = 'com.landray.kmss.km.imissive.model.KmImissiveSignMain';
        signData['contractName'] = subject;
        signData['signType'] = signType;
        signData['missiveName'] = 'com.landray.kmss.km.imissive.model.KmImissiveSignMain';
        ...
        //绑定
        our['bindOrgId'] = 'editonline'; //fdKey
    ...
    <c:if test="${(kmImissiveSignMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.eqbSign =='true' || kmImissiveSignMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.ofdEqbSign =='true')  && eqbFlag=='true'}">
	...
	<c:if test="${kmImissiveSignMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.yqqSignzq =='true' && yqqFlag=='true'}">
	 ...
	 Com_Parameter.event["submit"].push(function(){
		...
		if($(canStartProcess).val() == "true" && lbpm.globals.getCurrentOperation().operation && lbpm.globals.getCurrentOperation().operation['isPassType'] == true){
			var flag = true;
		 	var url = Com_Parameter.ContextPath+"km/imissive/km_imissive_out_sign/kmImissiveOutSign.do?method=queryCfcaFinish&signId=${param.fdId}";
		 	$.ajax({
				url : url,
				type : 'post',
				data : {},
...
seajs.use(['sys/ui/js/dialog'], function(dialog) {
	window.editDocNum = function(){
	    ...
	    dialog.iframe(url,'<bean:message  bundle="km-imissive" key="kmImissiveSignMain.editdocnum"/>',function(rtn){
	  ...
	  var values = [];
		...
		$.post(url,$.param({"List_Selected":values},true),function(data){
			if(data!=null && data.status==true){
				dialog.alert('<bean:message  bundle="km-imissive" key="msg.filing.success"/>');
			}else{
				dialog.alert('<bean:message  bundle="km-imissive" key="msg.filing.failure"/>');
			}
		},'json');
	},
				...
				if(data == true){
					...
					var url = Com_Parameter.ContextPath+"km/imissive/km_imissive_out_sign/kmImissiveOutSign.do?method=queryCfcaNeedContent";
				 	$.ajax({
						url : url,
						type : 'post',
						data : {"taskId":"${param.fdId}","taskName":"sign"},
						dataType : 'text',
						async : false,
						dataType:"json",
						error : function(){
							...
							if(data == true){
								var url = Com_Parameter.ContextPath+"km/imissive/km_imissive_out_sign/kmImissiveOutSign.do?method=queryCfcaStatusPdf";
								 $.ajax({
										url : url,
										type : 'post',
										data : {"fdId":"${param.fdId}","fdName":"sign"},
										dataType : 'text',
										async : false,
										dataType:"json",
										error : function(){
										...
										success:function(data){
											if(data == true){
												var infoUrl = "/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=openYqqExtendInfo&signId=${param.fdId}&signFlag=cfca";
												var extendIframe=dialog.iframe(infoUrl,"签署平台签署",null,{width:1200, height:600, topWin : window, close: true});
											}else{
...
function PrintDocument(){
  ...
  // tagElement.className = 'print';                                                 //样式改变为可打印
   var mCount = kmImissiveSignMainForm.SignatureControl.PrintDocument(false,2,5);  //打印控制窗体
  // alert("实际打印份数："+mCount);
  // tagElement.className = 'Noprint';                                               //样式改变为不可打印
}
<c:if test="${JGSignatureHTMLEnabled == 'true'}">
window.onload=function(){
	kmImissiveSignMainForm.SignatureControl.ShowSignature('${param.fdId}');
};
...
function generateFileNum(){
	    ...
	    path=Com_GetCurDnsHost()+Com_Parameter.ContextPath+'km/imissive/km_imissive_sign_main/kmImissiveNum.jsp?fdId=${kmImissiveSignMainForm.fdId}&fdNumberId=${fdNoId}&fdTemplateId=${kmImissiveSignMainForm.fdTemplateId}';
	    dialog.iframe(path,'<bean:message  bundle="km-imissive" key="kmImissive.modifyDocNum"/>',function(rtn){
...
function confirmSupervisedInner(){
	var url = "${KMSS_Parameter_ContextPath}km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=updateConfirmSupervise"
	 $.ajax({     
	     type:"post",     
	     url:url,     
	     data:{fdModelId:"${kmImissiveSignMainForm.fdId}",fdModelName:"com.landray.kmss.km.imissive.model.KmImissiveSignMain"},     
	     async:false,    //用同步方式 
	     success:function(data){
	    	    results =  eval("("+data+")");
...
function confirmSupervised(){
	if("${kmImissiveSignMainForm.fdSuperviseFlag}" == 'true'){
...
function  Com_submitReviewInner(){
    ...
    var canStartProcess = document.getElementById("sysWfBusinessForm.canStartProcess");
    if("${fdNoId}"!="" && (checkIfFutureNodeSelected() || (canStartProcess && canStartProcess.value == "false"))){
		 deleteBufferNumByNumberId();
	 }
}	


<c:if  test="${kmImissiveSignMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.confirmSupervise =='true' and not empty fdSuperviseId}">
Com_Parameter.event["submit"].push(function(){
    var canStartProcess = document.getElementById("sysWfBusinessForm.canStartProcess");
    if($(canStartProcess).val() == "true" && lbpm.globals.getCurrentOperation().operation && lbpm.globals.getCurrentOperation().operation['isPassType'] == true){
        var fdSuperviseFlag = '${kmImissiveSignMainForm.fdSuperviseFlag}';
...
<c:if test="${kmImissiveSignMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.ofdCySign =='true'}">
    Com_Parameter.event["submit"].push(function () {
        var canStartProcess = document.getElementById("sysWfBusinessForm.canStartProcess");
        //操作类型为通过类型 ，才判断是否已经签署
        if ($(canStartProcess).val() == "true" && lbpm.globals.getCurrentOperation().operation && lbpm.globals.getCurrentOperation().operation['isPassType'] == true) {
...
<c:if test="${kmImissiveSignMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.pdfJgSign =='true'}">
		   ...
		   if(docPublishTime.length > 0){
		    ...
		    var url="${KMSS_Parameter_ContextPath}km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=saveSignatureAndTime"; 
		   	 $.ajax({     
	    	     type:"post",
	    	     url:url,     
	    	     data:{fdId:"${kmImissiveSignMainForm.fdId}",docPublishTime:docPublishTimeVal},    
	    	     async:false,     //用同步方式 
	    	     success:function(data){
	    	    	 results =  eval("("+data+")");
		    	    if(results['flag']=="true"){
