...
<template:include file="/sys/profile/resource/template/list.jsp">
	<template:replace name="content">  
		 <list:criteria id="signCriteria">
			...
			<list:cri-criterion title="${ lfn:message('km-imissive:kmImissiveSignMain.docStatus')}" key="docStatus" multi="false"> 
				<list:box-select>
					<list:item-select>
			...
			<list:cri-auto modelName="com.landray.kmss.km.imissive.model.KmImissiveSignMain" property="fdDocNum;fdDrafter;fdDraftTime;docPublishTime;fdIsFiling" />
		</list:criteria>
				...
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar count="4" id="Btntoolbar">
						...
						<c:if test="${param.nodeType=='TEMPLATE'}">
							<kmss:auth
								requestURL="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=add&fdTemplateId=${param.categoryId}"
								requestMethod="GET">
								<ui:button text="${lfn:message('button.add')}" onclick="Com_OpenWindow('${LUI_ContextPath }/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=add&fdTemplateId=${JsParam.categoryId}');" order="2"></ui:button>
							</kmss:auth>
						</c:if>	
						<kmss:auth
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=manageList&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}'}
			</ui:source>
			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
				...
				window.addDoc = function() {
					dialog.categoryForNewFile(
							...
							'/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=add&fdTemplateId=!{id}',false,null,null,getValueByHash("fdTemplate"),null,null,true);
				};
				//删除
				window.delDoc = function(){
					...
					var url='<c:url value="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=deleteall"/>&categoryId=${param.categoryId}&nodeType=${param.nodeType}';
					var config = {
						url : url, // 删除数据的URL
	            ...
	            window.showFilingBtn = function(cateId,nodeType){
        			 var checkDelUrl ="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do?method=filingall&categoryId="+cateId+"&nodeType="+nodeType;
					 var data = new Array();
