			...
			<c:choose>
				...
				<c:otherwise>
					...
					<c:set var="circulationUrl"
						value="/km/imissive/km_imissive_circulation/kmImissiveCirculation.do?method=add&fdModelName=${sysCirculationForm.circulationForm.fdModelName}&fdModelId=${sysCirculationForm.circulationForm.fdModelId}&fdIsNewVersion=${sysCirculationForm.circulationForm.fdIsNewVersion}&mainDocStatus=${sysCirculationForm.docStatus}" />
					...
					<c:if test="${showButton eq 'false' and  isNewVersion}">
						<kmss:auth requestURL="/sys/circulation/sys_circulation_main/sysCirculationMain.do?method=addCirculate&fdModelName=${sysCirculationForm.circulationForm.fdModelName}&fdModelId=${sysCirculationForm.circulationForm.fdModelId}&aa=bb" requestMethod="GET">
							<c:set var="showButton" value="true"></c:set>
		...
		<c:if test="${showOption == 'all' || empty showOption }">
			...
			<div data-dojo-type="mui/tabbar/TabBarButton"
				data-dojo-props='icon1:"",align:"${param.align}",label:"${_cir_label}",
					href:"/sys/circulation/mobile/index.jsp?modelName=${kmReviewMainForm.modelClass.name}&modelId=${kmReviewMainForm.fdId}&fdKey=${param.fdKey}&isNew=${param.isNew}&modelKey=imissive"'>
			</div>
			</c:if>
		</c:otherwise>
	</c:choose>
