...
<template:include ref="mobile.list" canHash="true">
	<template:replace name="title">
		...
		<c:if test="${param.moduleName==null || param.moduleName==''}">
			<c:out value="${lfn:message('km-imissive:kmImissive.tree.all')}"></c:out>
		</c:if>
	</template:replace>
	<template:replace name="head">
        ...
        <mui:cache-file name="mui-kmImissive-imissiveLibrary.js" cacheType="md5"/>
       	<mui:cache-file name="mui-module.css" cacheType="md5" />
	</template:replace>
	<template:replace name="content">

		 <c:import url="/km/imissive/mobile/imissive_library/listview.jsp" charEncoding="UTF-8"></c:import>
