...
<c:set var="definedId" value="${fdNoId}" scope="request"/>
<div data-dojo-type="mui/view/DocScrollableView" id="_modifyDocNumView">
	...
	<div data-dojo-type="mui/panel/Content">
		<div class="muiFormContent">
			<table class="muiSimple">
			   <tr>
					<td>
						<div  style='font-size:14px;'>编号规则:</div>
						<%
						...
						%>
						<xform:select property="fdNumberId"  mobile="true"   showStatus="edit" value="${fdNoId}" htmlElementProperties="id='numberSelect'" style="width:200px">
	...
	<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" data-dojo-props='fill:"grid"'>
		<li data-dojo-type="mui/tabbar/TabBarButton" class="muiBtnNext "  onclick="optGetNum();">预览</li>
...
<script language="JavaScript">
	...
	function(ready,registry,topic,query,domStyle,array,domClass,lang,Tip,req,adapter,util,device,TransitionEvent,locale){
		ready(function(){
			//var docId = "${kmImissiveSendMainForm.fdTemplateId}";
			//generateNum(docId);
		});
		window.generateNum= function(docId){
		   if("${fdNoId}"!=""){
				optGetNum();
		   }
		}
		...
		window.getUnUseNum= function(){
			...
			req(util.formatUrl("/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=getUnUseNum"), {
				method : 'post',
				data:{fdNumberId:fdNumberId,fdId:"${kmImissiveSendMainForm.fdId}",fdTemplateId:"${kmImissiveSendMainForm.fdTemplateId}"}
	 ...
	 <c:if  test="${kmImissiveSendMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.modifyDocNum =='true' and kmImissiveSendMainForm.docStatus eq '20'}">
		 	...
		 	if($(canStartProcess).val() == "true" &&  lbpm.globals.getCurrentOperation().operation && lbpm.globals.getCurrentOperation().operation['isPassType'] == true){
		 	   ...
		 	   var fdYearNo = document.getElementsByName("fdYearNo")[0];
		 	    ...
		 	    if(""==docNum.value){
			 	   ...
			 	   req(util.formatUrl("/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=saveDocNum"), {
						method : 'post',
						data:{fdDocNum:docNum.value,fdFlowNo:fdFlowNo.value,fdNoRule:fdNoRule.value,fdNumberMainId:fdNumberMainId.value,fdId:"${kmImissiveSendMainForm.fdId}",fdYearNo:fdYearNo.value}
				   }).then(lang.hitch(this, function(data) {
					results =  eval("("+data+")");
			   ...
			   req(util.formatUrl("/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=saveSignatureAndTime"), {
					...
					results =  eval("("+data+")");
				    	if(results['flag']=="false"){
				    		Tip.tip({icon:'mui mui-warn', text:'生成签发日期失败'});
				    		flag = false;
				    	}
				}));
			   return flag;
