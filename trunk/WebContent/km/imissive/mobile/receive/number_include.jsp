...
require(['dojo/ready',
	...
	ready(function(){
		//var docId = "${kmImissiveReceiveMainForm.fdId}";
		//generateNum(docId);
	});
	...
	window.getUnUseNum= function(){
		...
		req(util.formatUrl("/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=getUnUseNum"), {
			method : 'post',
			data:{fdNumberId:fdNumberId,fdId:"${kmImissiveReceiveMainForm.fdId}",fdTemplateId:"${kmImissiveReceiveMainForm.fdTemplateId}"}
	...
	window.optGetNum= function(){
	 ...
	 <c:if  test="${kmImissiveReceiveMainForm.sysWfBusinessForm.fdNodeAdditionalInfo.modifyDocNum =='true'  and kmImissiveReceiveMainForm.docStatus eq '20'}">
		 	...
		 	if($(canStartProcess).val() == "true" && lbpm.globals.getCurrentOperation().operation && lbpm.globals.getCurrentOperation().operation['isPassType'] == true){
		 	   ...
		 	   var fdYearNo = document.getElementsByName("fdYearNo")[0];
		 	    ...
		 	    if(""==docNum.value){
			 	    	...
			 	    	req(util.formatUrl("/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=saveDocNum"), {
						method : 'post',
