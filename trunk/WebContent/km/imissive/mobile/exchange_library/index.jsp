...
<template:include ref="mobile.list" canHash="true">
	<template:replace name="title">
		...
		<c:if test="${param.moduleName==null || param.moduleName==''}">
			<c:out value="${lfn:message('km-imissive:kmImissive.tree.Exchange')}"></c:out>
		</c:if>
	</template:replace>
	<template:replace name="head">
	    <mui:cache-file name="mui-nav.js" cacheType="md5"/>
		<mui:cache-file name="mui-sysCate.js" cacheType="md5" />
        <mui:cache-file name="mui-kmImissive-exchangeLibrary.js" cacheType="md5"/>
	</template:replace>
	<template:replace name="content">
		 <c:import url="/km/imissive/mobile/exchange_library/listview.jsp" charEncoding="UTF-8"></c:import>
