...
<template:include ref="mobile.list" canHash="true">
	<template:replace name="title">
		...
		<c:if test="${param.moduleName==null || param.moduleName==''}">
			<c:out value="${lfn:message('km-imissive:kmImissive.tree.myapproved')}"></c:out>
		</c:if>
	</template:replace>
	<template:replace name="head">
        ...
        <mui:cache-file name="mui-kmImissive-imissiveApprovaled.js" cacheType="md5"/>
        <mui:cache-file name="mui-module.css" cacheType="md5" />
	</template:replace>
	<template:replace name="content">

		 <c:import url="/km/imissive/mobile/imissive_approvaled/listview.jsp" charEncoding="UTF-8"></c:import>
