...
<tr>
	<td class="td_normal_title">
		<bean:message key="kmImissiveStat.fdDateType" bundle="km-imissive"/>
	</td>
	<td colspan="3">
		...
		<xform:radio property="fdDateType" onValueChange="chgTimeSelect" value="${queryForm.fdDateType }" showStatus="edit">
		   <c:choose>
		       <c:when test="${JsParam.selfDefine==false}">
		            <xform:enumsDataSource enumsType="km_imissive_stat_fd_date_type_withOutDay">
					</xform:enumsDataSource>
		        </c:when>
		       <c:otherwise>
		        	<xform:enumsDataSource enumsType="km_imissive_stat_fd_date_type">
					</xform:enumsDataSource>
		       </c:otherwise>
		   </c:choose>
		</xform:radio>
...
<tr id="timeScope_5" name="timeScope">
	<td class="td_normal_title">
		<bean:message key="kmImissiveStat.fdDateType.scope" bundle="km-imissive"/>
	</td>
	<td colspan="3">
		...
		<bean:message  bundle="km-imissive" key="kmImissiveStat.fdDateType.to"/>
...
<tr style="display:none" id="timeScope_7" name="timeScope">
	...
	<td colspan="3">
		...
		<div class="inputselectsgl" onclick="selectDate('fdStartDate_7',null,setCurrTimeVal,setCurrTimeVal);" style="width:20%">
			<div class="input"><input type="text" name="fdStartDate_7" value="${queryForm.fdStartDate }" subject='<bean:message  bundle="km-imissive" key="kmImissiveStat.fdStartDate"/>'></div>
			<div class="inputdatetime"></div>
		</div>
		<span style="position:relative;top:-7px;"><bean:message  bundle="km-imissive" key="kmImissiveStat.fdDateType.to"/></span>
		<div class="inputselectsgl" onclick="selectDate('fdEndDate_7',null,setCurrTimeVal,setCurrTimeVal);" style="width:20%">
			<div class="input"><input type="text" name="fdEndDate_7" value="${queryForm.fdEndDate }" subject='<bean:message  bundle="km-imissive" key="kmImissiveStat.fdEndDate"/>'></div>
			<div class="inputdatetime"></div>
		</div>
	</td>
</tr>
