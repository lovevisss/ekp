...
<template:include file="/sys/profile/resource/template/list.jsp">
	<template:replace name="content">  
		<list:criteria id="sendCriteria">
			...
			<list:cri-criterion title="${ lfn:message('km-imissive:kmImissiveSendMain.docStatus')}" key="docStatus"  multi="false"> 
				<list:box-select>
					<list:item-select>
						<ui:source type="Static">
							[{text:'${ lfn:message('status.draft')}', value:'10'},{text:'${ lfn:message('status.examine')}',value:'20'},{text:'${ lfn:message('km-imissive:kmImissive.status.refuse')}',value:'11'},{text:'${ lfn:message('km-imissive:kmImissive.status.discard')}',value:'00'},{text:'${ lfn:message('status.publish')}',value:'30'}]
						</ui:source>
					</list:item-select>
				</list:box-select>
			</list:cri-criterion>
					...
					<div style="float:right">
						...
						<ui:toolbar count="4" id="Btntoolbar">
							...
							<c:if test="${param.nodeType=='TEMPLATE'}">
								<kmss:auth
									requestURL="/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=add&fdTemplateId=${JsParam.categoryId}&fdModelId=${JsParam.fdModelId}&fdModelName=${JsParam.fdModelName}"
									requestMethod="GET">
									<ui:button title="${lfn:message('button.add')}" text="${lfn:message('button.add')}" onclick="Com_OpenWindow('${LUI_ContextPath }/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=add&fdTemplateId=${JsParam.categoryId}');" order="2"></ui:button>
								</kmss:auth>
							</c:if>	 
							<kmss:auth
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=manageList&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}'}
			</ui:source>
	 	...
	 	var SYS_SEARCH_MODEL_NAME ="com.landray.kmss.km.imissive.model.KmImissiveSendMain";
			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
				...
				window.delDoc = function(){
					...
					/* 软删除配置 */
					var url='<c:url value="/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=deleteall"/>&categoryId=${param.categoryId}&nodeType=${param.nodeType}';
					var config = {
						url : url, // 删除数据的URL
