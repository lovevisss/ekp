	  ...
	  <ui:tabpanel id="tabpanelDiv" layout="sys.ui.tabpanel.view" var-count="4" var-average = 'false'>
			  <ui:content id="kmImissiveSendMain_baseinfo" titleicon="lui-tab-icon tab-icon-02" title="${ lfn:message('km-imissive:kmImissiveSendMain.baseinfo') }" expand="${expandBaseInfo}">
			  	<c:import url="/km/imissive/km_imissive_send_main/kmImissiveSendMain_baseInfo.jsp" charEncoding="UTF-8">
			  		...
			  		<c:param name="formId" value="${kmImissiveSendMainForm.fdId}" />
			  	</c:import>
...
<template:replace name="barRight">
	...
	<c:if test="${not empty kmImissiveSendMainForm.attachmentForms['attachment'].attachments or not empty kmImissiveSendMainForm.attachmentForms['editonline'].attachments or not empty kmImissiveSendMainForm.attachmentForms['mainonline'].attachments or editAtt eq true}">
		<c:set var="showRightBar" value="true"></c:set>
	</c:if>
	...
	<c:choose>
		<c:when test="${showRightBar eq true}">
			 ...
			 <c:if test="${not empty kmImissiveSendMainForm.attachmentForms['editonline'].attachments or not empty kmImissiveSendMainForm.attachmentForms['mainonline'].attachments}">
				  <ui:content  title="${ lfn:message('km-imissive:kmImissiveSendMain.docContent') }" id="attContent">
			  ...
			  <c:if test="${not empty kmImissiveSendMainForm.attachmentForms['attachment'].attachments or editAtt eq true or not empty fdSignKey}">
				   <ui:content   title="${ lfn:message('sys-doc:sysDocBaseInfo.docAttachments') }">
						...
						<c:choose>
							...
							<c:if test="${not empty kmImissiveSendMainForm.attachmentForms[fdSignKey].attachments}">
