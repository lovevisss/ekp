...
function addAuditShowBookMarks(attKey){
	...
	addAuditShowInfo(_bookmarks,office,"${kmImissiveSendMainForm.fdId}","${kmImissiveSendMainForm.extendDataFormInfo.extendFilePath}");
}
...
function addBookMarks(attKey){
	...
	Attachment_ObjectInfo[attKey].setBookmark('signature',fdSignatureName);
	...
	Attachment_ObjectInfo[attKey].setBookmark('draftunit', fdDraftUnitName);
	...
	Attachment_ObjectInfo[attKey].setBookmark('draftdept', fdDraftDeptName);
	...
	//	fdMaintoNames = fdMaintoNames.replace(new RegExp(";",'g'),"、");
	//}
	Attachment_ObjectInfo[attKey].setBookmark('maintounit', fdMaintoNames);
	...
	//	fdCopytoNames = fdCopytoNames.replace(new RegExp(";",'g'),"、");
	//}
	Attachment_ObjectInfo[attKey].setBookmark('copytounit', fdCopytoNames);
	...
	//	fdReporttoNames = fdReporttoNames.replace(new RegExp(";",'g'),"、");
	//}
	Attachment_ObjectInfo[attKey].setBookmark('reporttounit', fdReporttoNames);
	...
	//	fdMissiveMpersonNames = fdMissiveMpersonNames.replace(new RegExp(";",'g'),"、");
	//}
	Attachment_ObjectInfo[attKey].setBookmark('maintoperson', fdMissiveMpersonNames);
	...
	//	fdMissiveCpersonNames = fdMissiveCpersonNames.replace(new RegExp(";",'g'),"、");
	//}
	Attachment_ObjectInfo[attKey].setBookmark('copytoperson', fdMissiveCpersonNames);
	...
	//	fdMissiveRpersonNames = fdMissiveRpersonNames.replace(new RegExp(";",'g'),"、");
	//}
	Attachment_ObjectInfo[attKey].setBookmark('reporttoperson', fdMissiveRpersonNames);
	...
	Attachment_ObjectInfo[attKey].setBookmark('printunit', fdPrintUnitName);
	...
	Attachment_ObjectInfo[attKey].setBookmark('printtime', fdPrintTimeNum);
	...
	Attachment_ObjectInfo[attKey].setBookmark('printtimecn', fdPrintTimeUpper);
	...
	if(fdPrintTimeE.length > 0 && fdPrintTimeE[0].value !=''){
		...
		var url="${KMSS_Parameter_ContextPath}km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=transferTime"; 
		 $.ajax({     
		     ...
		     success:function(rtn){
		 	   if(rtn['fdPrintTimeUpper']){
		 		  Attachment_ObjectInfo[attKey].setBookmark('printtimecn', rtn['fdPrintTimeUpper']);
		 	   }
		 	  if(rtn['fdPrintTimeNum']){
		 		 Attachment_ObjectInfo[attKey].setBookmark('printtime', rtn['fdPrintTimeNum']);
		 	   }
			}   
	   });
	}else{
	...
	Attachment_ObjectInfo[attKey].setBookmark('printtimen', fdPrintTime);
	...
	Attachment_ObjectInfo[attKey].setBookmark('level', fdLevel);
	...
	Attachment_ObjectInfo[attKey].setBookmark('version', fdVersion);
	...
	Attachment_ObjectInfo[attKey].setBookmark('secretyear', fdSecretYear);
