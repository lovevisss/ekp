...
<script>
	function downloadPdf(){
		var url = "${LUI_ContextPath}/sys/attachment/sys_att_main/sysAttMain.do?method=downloadPdf&fdId=${fdAttMainId}&fdFileName="+encodeURI('${kmImissiveSendMainForm.docSubject}')+"&convertType=${_isPdfServiceName}";
		Com_OpenWindow(url,'_blank');
	}
	function downloadOfd(){
		var url = "${LUI_ContextPath}/sys/attachment/sys_att_main/sysAttMain.do?method=downloadOfd&fdId=${fdAttMainId}&fdFileName="+encodeURI('${kmImissiveSendMainForm.docSubject}')+"&convertType=${_isOfdServiceName}";
		Com_OpenWindow(url,'_blank');
	}
</script>
<%
...
%>
		<kmss:auth requestURL="/sys/attachment/sys_att_main/sysAttMain.do?method=downloadPdf&fdModelName=com.landray.kmss.km.imissive.model.KmImissiveSendMain&fdId=${fdAttMainId}" requestMethod="GET">
...
%>
		<kmss:auth requestURL="/sys/attachment/sys_att_main/sysAttMain.do?method=downloadOfd&fdModelName=com.landray.kmss.km.imissive.model.KmImissiveSendMain&fdId=${fdAttMainId}" requestMethod="GET">
			 <a href="javascript:void(0);" class="attswich"
					onclick="downloadOfd();">
			   下载ofd
			 </a>
...
<c:if test="${_isWpsCloudEnable ne true && _isWpsCenterEnable ne true and _useWpsLinuxView ne true and empty restDownloadUrl and 'true' ne _useViewDowload}">
 <%if(JgWebOffice.isJGEnabled()&&JgWebOffice.isExistViewPath(request)){%>
	   <c:if test="${isIE or isJGMULEnabled}">
		    ...
		    <c:choose>
			     <c:when test="${isExistViewPath}">
			     	 <a href="javascript:void(0);" class="attswich com_btn_link"
						onclick="Com_OpenWindow('kmImissiveSendMain.do?method=view&fdId=${kmImissiveSendMainForm.fdId}&isShowImg=${isShowImg}&change=true','_self');">
