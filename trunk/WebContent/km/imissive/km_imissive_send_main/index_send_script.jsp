...
seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
	...
	window.expanded = function(){
		...
		LUI('sendCriteria').expandCriterions(!LUI('sendCriteria').expand);
		
	};
	
	window.doSearchAction = function(evt){
		var topicEvent = {
				criterions : LUI("sendCriteria")._buildCriteriaSelectedValues(),
				query : []
			};
		var obj = {};
		var values = [];
        ...
        var j_path_value;
        if("${param.contentId}" == "kmImissiveSendPContent"){
            j_path_value = "/listAllPublish/send";
        }
        if("${param.contentId}" == "kmImissiveSendCContent"){
            j_path_value = "/listCreate/send";
        }
        if("${param.contentId}" == "kmImissiveSendFContent"){
            j_path_value = "/filing/send";
        }
        if("${param.contentId}" == "kmImissiveSendDContent"){
            j_path_value = "/discard/send";
        }
        if("${param.contentId}" == "kmImissiveSendContent"){
            j_path_value = "/listAll/send";
        }
        if(j_path_value != ""){
            var obj2 = {};
            var values2 = [];
	...
	window.addOffLineDoc = function() {
		var url = "/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=addOffLine&fdTemplateId=!{id}";
		dialog.categoryForNewFile('com.landray.kmss.km.imissive.model.KmImissiveSendTemplate',
				url, false, null, null, cateId, null, null, true); 
	};
	
	window.addDoc = function() {
		/**
		* 其他模块引用该页面若需要在新建发文时带参数，按如下规则传值
		* 需要带过来参数（可多个）作为urlParam的值
		* urlParam的值中 "=" 用 ":" 代替，"&" 用 "@" 代替
		* 可参考会议管理index.jsp中的纪要查看
		*/
		var urlParam = "${param.urlParam}";  // 获取其他模块带过来的参数
		var url = "/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=add&fdTemplateId=!{id}";
		if (urlParam) {
			// 格式化路径参数
	...
	window.openSupervise = function(){
		...
		var url  = '${KMSS_Parameter_ContextPath}km/supervise/km_supervise_main/kmSuperviseMain.do?method=add&modelName=com.landray.kmss.km.imissive.model.KmImissiveSendMain&modelId='+values[0];
	...
	//批量打印
		...
		var url  = '<c:url value="/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=printBatch"/>'+'&fdIds='+values;
		Com_OpenWindow(url);
	};
	//删除
	window.delDoc = function(){
		...
		/* 软删除配置 */
		var url='<c:url value="/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=deleteall"/>'+'&categoryId='+cateId+'&nodeType='+nodeType;
		if(docStatus == '10'){
			var url='<c:url value="/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=deleteall&draft=true"/>'+'&categoryId='+cateId+'&nodeType='+nodeType;
		}
		var config = {
			url : url, // 删除数据的URL
	 ...
	 if(AuthCache[cateId]){
         ...
         //分类转移按钮
          ...
          window.showFilingBtn = function(cateId,nodeType){
     			 var checkDelUrl ="/sys/archives/sys_archives_file_template/sysArchivesFileTemplate.do?method=fileDocAll&modelName=com.landray.kmss.km.imissive.model.KmImissiveSendMain&categoryId="+cateId+"&nodeType="+nodeType;
		 var data = new Array();
	...
	<%
	   ...
	   request.setAttribute("delAll",UserUtil.checkAuthentication("/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=deleteall","get"));
	   request.setAttribute("chgCateAll",UserUtil.checkAuthentication("/km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=changeTemplate","get"));
	   request.setAttribute("changeRightBatchAll",UserUtil.checkAuthentication("/sys/right/cchange_doc_right/cchange_doc_right.jsp?modelName=com.landray.kmss.km.imissive.model.KmImissiveSendMain","get"));
	%>
