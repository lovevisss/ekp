...
$(document).ready(function(){
	...
	if(method == "add"){
		if($('input:radio[name="sysFormTemplateForms.signMainDoc.fdMode"]')){
		...
		if($('select[name="sysFormTemplateForms.signMainDoc.fdMode"]')){
			$('select[name="sysFormTemplateForms.signMainDoc.fdMode"]').children(":first").remove();
			$('select[name="sysFormTemplateForms.signMainDoc.fdMode"]').children(":nth-child(1)").prop("selected","selected");
		}
...
Com_Parameter.event["submit"].push(function(){
	...
	function submitForm(method) {
		if(typeof XForm_BeforeSubmitForm != 'undefined' && XForm_BeforeSubmitForm instanceof Function){
			XForm_BeforeSubmitForm(function(){
				Com_Submit(document.kmImissiveSignTemplateForm, method);
			});
		}else{
			Com_Submit(document.kmImissiveSignTemplateForm, method);
	    }
	...
	function openBookmarkHelp(){
		datas = [];
		var baseObjs;
		var fdType = $('select[name="sysFormTemplateForms.signMainDoc.fdMode"]').find("option:selected").val();
		if(fdType == '2'){
			var fdTempId = document.getElementsByName("sysFormTemplateForms.signMainDoc.fdCommonTemplateId")[0].value;
			baseObjs = _XForm_GetCommonExtDictObj(fdTempId);
		}else{
			var customIframe = window.frames['IFrame_FormTemplate_signMainDoc'].contentWindow;
			if(!customIframe){
				customIframe = window.frames['IFrame_FormTemplate_signMainDoc'];
			}
			if (customIframe.Designer && customIframe.Designer.instance && customIframe.Designer.instance.hasInitialized) {
				...
				var xformTemplateId = document.getElementsByName("sysFormTemplateForms.signMainDoc.fdCommonTemplateId")[0].value;
		...
		Com_OpenWindow(Com_Parameter.ContextPath+'km/imissive/km_imissive_sign_main/bookMarks.jsp','_blank');
	}	
</script>
				...
				<table class="tb_normal" width=100%>
					...
					<td width=35%>
						...
						<label class="weui_switch acrossYearNumber">
							<span class="weui_switch_bd">
								<input type="checkbox" ${'true' eq kmImissiveSignTemplateForm.fdAcrossYearNumber ? 'checked' : '' } />
								<span></span>
								<small></small>
							</span>
						...
						<script type="text/javascript">
							...
							setYearText(${kmImissiveSignTemplateForm.fdAcrossYearNumber});
						</script>
					</td>
					<td class="td_normal_title" width=15%>
					...
					<td width=35%>
						...
						<label class="weui_switch available">
							<span class="weui_switch_bd">
								<input type="checkbox" ${'true' eq kmImissiveSignTemplateForm.fdIsAvailable ? 'checked' : '' } />
								<span></span>
								<small></small>
							</span>
						...
						<script type="text/javascript">
							...
							setText(${kmImissiveSignTemplateForm.fdIsAvailable});
						</script>
					</td>
				</tr>
				<%-- 所属场所 --%>
										...
										<c:choose>
											<c:when test="${'true' eq xinChuangWps}">
												<jsp:include page="/km/imissive/wps/oaassist/sysAttMain_edit.jsp">
													...
													<jsp:param name="fdModelId" value="${kmImissiveSignTemplateForm.fdId}"/>
													<jsp:param name="fdModelName" value="com.landray.kmss.km.imissive.model.KmImissiveSignTemplate"/>
													<jsp:param name="formBeanName" value="kmImissiveSignTemplateForm"/>
