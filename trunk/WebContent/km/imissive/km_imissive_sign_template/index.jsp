...
<template:include file="/sys/profile/resource/template/list.jsp">
	<template:replace name="content">
		...
		<list:criteria id="criteria">
		     <list:cri-ref key="fdName" ref="criterion.sys.docSubject" title="${lfn:message('km-imissive:kmImissiveSignTemplate.fdName') }">
			</list:cri-ref>
			<%-- 搜索条件:是否有效 --%>
			...
			<div class="lui_list_operation_sort_btn">
				...
				<div class="lui_list_operation_sort_toolbar">
					<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" >
					    <list:sortgroup>
						    <list:sort property="fdOrder" text="${lfn:message('km-imissive:kmImissiveSignTemplate.fdOrder') }" group="sort.list" value="up"></list:sort>
							<list:sort property="docCreateTime" text="${lfn:message('km-imissive:kmImissiveSignTemplate.docCreateTime') }" group="sort.list" ></list:sort>
							<list:sort property="fdName" text="${lfn:message('km-imissive:kmImissiveSignTemplate.fdName') }" group="sort.list"></list:sort>
						</list:sortgroup>
					</ui:toolbar>
				</div>
			</div>
			...
			<div style="float:right">
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar id="Btntoolbar" count="2">
					    <kmss:auth requestURL="/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do?method=save&fdCategoryId=${param.parentId}" requestMethod="GET">
							<ui:button text="${lfn:message('button.add')}"  onclick="add();" order="1" ></ui:button>
						</kmss:auth>
						<kmss:auth requestURL="/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do?method=deleteall&parentId=${param.parentId}" requestMethod="GET">
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do?method=listChildren&parentId=${param.parentId}&ower=1'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
			    rowHref="/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do?method=view&fdId=!{fdId}">
	 	...
	 	<script type="text/javascript">
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
		 		...
		 		window.addDoc = function(fdId) {
		 			Com_OpenWindow('<c:url value="/km/imissive/km_imissive_sign_main/kmImissiveSignMain.do" />?method=add&fdTemplateId='+fdId);
		 		};
			 	// 增加
		 		window.add = function() {
		 			Com_OpenWindow('<c:url value="/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do" />?method=add&parentId=${param.parentId}');
		 		};
		 	    // 编辑
		 		window.edit = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do" />?method=edit&fdId=' + id);
		 		};
		 		window.deleteAll = function(id){
					...
					var url = '<c:url value="/km/imissive/km_imissive_sign_template/kmImissiveSignTemplate.do?method=deleteall"/>';
