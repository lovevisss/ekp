        ...
        <table class="tb_normal" width="98%">
            <tr>
                ...
                <td width=75% colspan="3">
                    <xform:text property="fdName" showStatus="edit" style="width:70%"/>
                    <ui:button id="res_free_btn" text="${lfn:message('button.search') }"
                               onclick="queryResource('free')"/>
                </td>
            </tr>
            <tr>
                    <%--资源列表--%>
                <td colspan="4">
                    <list:listview id="listview" style="height:200px;overflow-y:scroll">
                        <ui:source type="AjaxJson">
                            {url:'/km/imeeting/km_imeeting_equipment/kmImeetingEquipment.do?method=listEquipment&type=free&rowsize=3&fdHoldDate=${JsParam.fdHoldDate}&fdFinishDate=${JsParam.fdFinishDate}&meetingId=${JsParam.meetingId}'}
                        </ui:source>
                        <list:colTable isDefault="false" layout="sys.ui.listview.listtable" name="columntable"
                                       onRowClick="selectResouce('!{fdId}');">
                            <list:col-html title=" " style="text-align:left;width:10%;">
                                {$ <input type="checkbox" value="{%row['fdId']%}" name="equipmentId"
                                          data="{%row['fdName']%}"> $}
                            </list:col-html>
            ...
            <tr>
                ...
                <td colspan="4" style="text-align: center;">
                        <%--选择--%>
                    <ui:button id="select_resource_submit" text="${lfn:message('button.select')}"
                               onclick="selectSubmit();"/>
                        <%--取消--%>
                    <ui:button id="select_resource_cancel" text="${lfn:message('button.cancel') }"
                               onclick="selectCancel();"/>
                        <%--取消选定--%>
                    <ui:button id="select_resource_cancel" text="${lfn:message('button.cancelSelect') }"
                               onclick="selectCancelSubmit();"/>
                </td>
            </tr>
        </table>
    </template:replace>
