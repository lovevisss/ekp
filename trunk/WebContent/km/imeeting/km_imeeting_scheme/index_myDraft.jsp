    ...
    <template:replace name="body">
                ...
                <div style="display: inline-block;vertical-align: middle;">
                    <ui:toolbar count="11" id="Btntoolbar">
                        ...
                        <c:if test="${not empty JsParam.fdTemplateId}">
                            <kmss:auth
                                    requestURL="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=add&fdTemplateId=${JsParam.fdTemplateId}">
                                <ui:button text="${lfn:message('button.add')}" onclick="addDoc()" order="1"></ui:button>
                            </kmss:auth>
                        </c:if>
                        <c:if test="${empty JsParam.docCategoryId}">
                            <kmss:auth requestURL="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=add">
                                <ui:button text="${lfn:message('button.add')}" onclick="addDoc()" order="1"></ui:button>
                            </kmss:auth>
                        </c:if>
        ...
        <list:listview id="listview_send">
            <ui:source type="AjaxJson">
                {url:'/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=list&q.docType=myDraft&isDialog=0&q.j_path=%2Findex_myDraft'}
            </ui:source>
            <list:colTable
