...
<template:include ref="default.print" sidebar="no">
	...
	<template:replace name="content">
		...
		<form name="kmImeetingSchemeFormTemp" method="post" action="<c:url value="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do" />">
		<center>
		...
		<center id="print_content">
			<c:forEach items="${kmImeetingSchemeFormList }" var="kmImeetingSchemeFormTemp" varStatus="vstatus">
				<kmss:auth requestURL="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=view&fdId=${kmImeetingSchemeFormTemp.fdId}" requestMethod="GET">
					<c:set var="kmImeetingSchemeForm" value="${kmImeetingSchemeFormTemp }" scope="request" />
					<div class="print_title_header">
						<p id="title" class="print_txttitle">
							<bean:write name="kmImeetingSchemeForm" property="docSubject" />
						</p>
						<div class="printDate">
									...
									<tr>
										<td name="_xform_detail">
											...
											<c:import url="/sys/xform/include/sysForm_view.jsp" charEncoding="UTF-8">
												...
												<c:param name="fdId" value="${kmImeetingSchemeForm.fdId }" />
					...
					<% 
						...
						// 清除表单信息
						request.removeAttribute("kmImeetingSchemeForm"); 
					%>
				</kmss:auth>
			</c:forEach>
		</center>
