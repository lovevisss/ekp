...
<template:include file="/sys/profile/resource/template/list.jsp">
    <template:replace name="content">
        ...
        <list:criteria id="imeetingCriteria">
            <%-- 方案状态 --%>
            <list:cri-criterion title="${ lfn:message('km-imeeting:kmImeetingScheme.docStatus') }" key="docStatus">
                <list:box-select>
                    <list:item-select id="status_for_myAttend" cfg-enable="true">
                    ...
                    <ui:toolbar layout="sys.ui.toolbar.sort" style="float:left">
                        <list:sortgroup>
                            <list:sort property="docCreateTime"
                                       text="${lfn:message('km-imeeting:kmImeetingMain.docCreateTime') }"
                                       group="sort.list" value="down"/>
                            <list:sort property="docPublishTime"
                                       text="${lfn:message('km-imeeting:kmImeetingMain.docPublishTime') }"
                                       group="sort.list"/>
                            <list:sort property="fdBeginDate"
                                       text="${lfn:message('km-imeeting:kmImeetingMain.fdHoldDate') }"
                                       group="sort.list"/>
                        </list:sortgroup>
                    </ui:toolbar>
                </div>
            ...
            <div style="float:right">
                <div style="display: inline-block;vertical-align: middle;">
                    <ui:toolbar count="4" id="Btntoolbar">
                        ...
                        <kmss:auth
                                requestURL="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=changeTemplate&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}"
                                requestMethod="GET">
                            <ui:button id="chgCate" text="${lfn:message('km-imeeting:kmImeeting.btn.translate')}"
                                       order="5" onclick="chgSelect();"/>
                        </kmss:auth>
                        <kmss:auth
                                requestURL="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=deleteall&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}"
                                requestMethod="GET">
                            <ui:button id="del" text="${lfn:message('button.deleteall')}" order="4"
                                       onclick="delDoc()"/>
                        </kmss:auth>
                        <%-- 收藏 --%>
        ...
        <list:listview id="listview">
            <ui:source type="AjaxJson">
                {url:'/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=manageList&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}'}
            </ui:source>
            <list:colTable
        ...
        <script type="text/javascript">
            var SYS_SEARCH_MODEL_NAME = "com.landray.kmss.km.imeeting.model.KmImeetingScheme";
            seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog', 'lui/topic', 'lui/toolbar'], function ($, strutil, dialog, topic, toolbar) {
                ...
                window.addDoc = function () {
                    dialog.categoryForNewFile(
                        'com.landray.kmss.km.imeeting.model.KmImeetingSchemeTemplate',
                        '/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=add&fdTemplateId=!{id}', false, null, null, categoryId);
                };
                //删除
                window.delDoc = function () {
                    ...
                    dialog.confirm('<bean:message key="page.comfirmDelete"/>', function (value) {
                        if (value == true) {
                            ...
                            $.post('<c:url value="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=deleteall"/>',
                    ...
                    var cfg = {
                        ...
                        'action': function (value, ____dialog) {
                            if (value && value.id) {
                                ...
                                $.post('<c:url value="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=changeTemplate"/>',
                ...
                var showButtons = function (categoryId, nodeType) {
                    var checkDelUrl = "/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=deleteall&categoryId=" + categoryId + "&nodeType=" + nodeType;
                    var checkChgCateUrl = "/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=changeTemplate&categoryId=" + categoryId + "&nodeType=" + nodeType;
                            ...
                            for (var i = 0; i < rtn.length; i++) {
                                ...
                                if (rtn[i]['changeRightBatchBtn'] == 'true') {
                                    var changeRightBatchBtn = toolbar.buildButton({
                                        ...
                                        click: 'changeRightCheckSelect("' + categoryId + '","' + nodeType + '")'
                                    });
