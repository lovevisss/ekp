...
<script type="text/javascript">
    ...
    seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog', 'lui/topic', 'lui/toolbar'], function ($, strutil, dialog, topic, toolbar) {
        ...
        LUI.ready(function () {
            if (getValueByHash("fdSchemeTemplate") != "") {
        ...
        window.addDoc = function () {
            dialog.categoryForNewFile(
                ...
                '/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=add&fdTemplateId=!{id}&.fdTemplate=!{id}&i.docTemplate=!{id}', false, null, null, cateId);
        };
        ...
        window.delDoc = function (draft) {
            ...
            var url = '${LUI_ContextPath }/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=deleteall';
            url = Com_SetUrlParameter(url, 'categoryId', cateId);
            url = Com_SetUrlParameter(url, 'nodeType', nodeType);
            if (draft == '0') {
                url = '${LUI_ContextPath }/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=deleteall&status=10';
            }
            var config = {
                url: url, // 删除数据的URL
                data: $.param({"List_Selected": values}, true), // 要删除的数据
                modelName: "com.landray.kmss.km.imeeting.model.KmImeetingScheme" // 主要是判断此文档是否有部署软删除
            };
        ...
        window.chgSelect = function () {
            ...
            var cfg = {
                ...
                <%
                    if(com.landray.kmss.util.UserUtil.checkRole("ROLE_KMIMEETING_SCHEME_OPTALL")) {
                %>
                'authType': '00',
                <%
                    } else {
        ...
        window.showButtons = function (cateId, nodeType) {
            ...
            if (AuthCache[cateId] == null) {
                var checkChgCateUrl = "/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=changeTemplate&categoryId=" + cateId + "&nodeType=" + nodeType;
                var checkDelUrl = "/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=deleteall&categoryId=" + cateId + "&nodeType=" + nodeType;
                var changeRightBatchUrl = "/sys/right/cchange_doc_right/cchange_doc_right.jsp?modelName=com.landray.kmss.km.imeeting.model.KmImeetingScheme&categoryId=" + cateId + "&nodeType=" + nodeType;
                var data = new Array();
        ...
        topic.subscribe('criteria.spa.changed', function (evt) {
            ...
            for (var i = 0; i < evt['criterions'].length; i++) {
                //获取分类id和类型
                if (evt['criterions'][i].key == "fdSchemeTemplate") {
        ...
        window.expanded = function () {
            LUI('schemeCriteria').expandCriterions(!LUI('schemeCriteria').expand);
        };

        // 搜索
        window.doSearchAction = function (evt) {
            var topicEvent = {
                criterions: LUI("schemeCriteria")._buildCriteriaSelectedValues(),
                query: []
            };
            var obj = {};
            var values = [];
        ...
        window.batchPrint = function () {
            ...
            var url = '<c:url value="/km/imeeting/km_imeeting_scheme/kmImeetingScheme.do?method=printBatch"/>' + '&fdIds=' + values;
            Com_OpenWindow(url);
        };
