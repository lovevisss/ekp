...
<script>
    ...
    seajs.use(['lui/topic', 'lui/dialog', 'km/imeeting/resource/js/seatPlan',
        ...
        function (topic, dialog, SeatPlan, Seat, SeatUtil, ElementList, PersonList) {
            ...
            var attendPersonData = [];// 出席人员信息

            var summaryPersonData = [];// 纪要人员信息

            var invitePersonData = []; // 邀请人员信息
            ...
            var seat = new Seat($('#seatTemplate'), {
                ...
                beforeRenderData: function (cell, d) {
                    ...
                    cell.removeClass('status_seat_up status_seat_down status_seat_left status_seat_right');
                    cell.removeClass('status_occupation status_speech');
            ...
            window.changeSeatTemplate = function (val) {
                ...
                dialog.confirm("切换座席模板将清空已经设置的数据！！！", function (flag) {
                    if (flag) {
                        if (val == "") {
                            templateData = [];
                            seat.setData({
            ...
            var elementList, hostList, participantList, summaryList, inviteList, attendList, reportersList, reporterList
            ...
            window.init = function () {
                ...
                // 初始化邀请人员
                inviteList = new PersonList($('#inviteTab'), [], {
                    onSelect: function (clazz) {
                        activeClass = clazz;
                    }
                });
            ...
            window.removePlanElement = function (elementId) {
                var index = planElement.indexOf(elementId);
                if (index > -1) {
            ...
            window.initPerson = function () {
                ...
                $.get(url).then(function (res) {
                    ...
                    // 邀请人员信息
                    $.each(res.invite || [], function (_, d) {
                        invitePersonData.push($.extend({}, d));
                        allPersonData.push($.extend({}, d));
                    });
                    inviteList.setData(invitePersonData);
                    if (invitePersonData.length <= 0) {
                        $("#inviteWrap").hide();
                    } else {
                        $("#inviteWrap").show();
                    }
                    ...
                    removePerson();
                    console.log("allPersonData", allPersonData);
                });
                    ...
                    if (d.clazz.type == "1" || d.clazz.type == "0") {
                        if (!d.clazz.elementId) {
                            ...
                            $.each(elementData, function (_, p) {
                                ...
                                var ind = planElement.indexOf(elementId);
                                if (ind == -1) {
                                    flag = false;
                                    return false;
                                }
