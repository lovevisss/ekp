...
<%
	...
	KmImeetingMainFeedbackForm kmImeetingMainFeedbackForm = (KmImeetingMainFeedbackForm)request
			.getAttribute("kmImeetingMainFeedbackForm");
	String docCreatorId = kmImeetingMainFeedbackForm.getDocCreatorId();
	String fdCurUserId = UserUtil.getKMSSUser().getUserId();
	if (fdCurUserId.equals(docCreatorId)) {
		request.setAttribute("fdIsCanSign", "true");
	} else {
		request.setAttribute("fdIsCanSign", "false");
	}
%>
<script>
			...
			<div class="lui_form_content_frame" style="padding-top:5px">
				...
				<c:if test="${shouldFeedback==true }">
					...
					<c:choose>
						...
						<c:otherwise>
							<%@include file="/km/imeeting/km_imeeting_main_feedback/kmImeetingMainFeedback_attend.jsp"%>
						</c:otherwise>
					</c:choose>
