seajs.use(['lui/framework/module','lui/jquery','lui/dialog','lui/topic','lang!km-imeeting','lui/util/env','km/imeeting/resource/js/dateUtil'],
	...
	module.controller(function($var, $lang, $function,$router){
		...
		$router.define({
			...
			
			{
				...
				action : function(){
					...
					createSummary(mainModelName, modelName, 'send', '/createSend');
				}
			},
			
			{
				...
				children : [{
				path:'/summaryMyTodo',   // 待我审的通知
				action : {
					type : 'tabpanel',
					options : {
							panelId : 'kmImeetingPanel',
							contents : {
								'summaryMyTodo' : { route:{ path: '/summaryApproval/summaryMyTodo' }, cri :{'docType':'myApproval', 'fdModelName':'com.landray.kmss.km.imeeting.model.KmImeetingMain', 'j_path':'/summaryApproval/summaryMyTodo'}, selected : true },
								'summaryMyDone' : { route:{ path: '/summaryApproval/summaryMyDone' }, cri :{'docType':'myApprovaled', 'fdModelName':'com.landray.kmss.km.imeeting.model.KmImeetingMain','j_path':'/summaryApproval/summaryMyDone'}}
							}
						}
					}
				},{
					...
					action : {
						...
						options : {
								panelId : 'kmImeetingPanel',
								contents : {
									'summaryMyTodo' : { route:{ path: '/summaryApproval/summaryMyTodo' }, cri :{'docType':'myApproval', 'fdModelName':'com.landray.kmss.km.imeeting.model.KmImeetingMain','j_path':'/summaryApproval/summaryMyTodo'}},
									'summaryMyDone' : { route:{ path: '/summaryApproval/summaryMyDone' }, cri :{'docType':'myApprovaled', 'fdModelName':'com.landray.kmss.km.imeeting.model.KmImeetingMain', 'j_path':'/summaryApproval/summaryMyDone'}, selected : true}
								}
							}
						}
				}]
				...
				action : {
					...
					options : {
						panelId : 'kmImeetingPanel',
						contents : {
							'summaryView' : { title : '纪要查看', route:{ path: '/summaryView' }, cri : buildCriParam($var.isCurAdmin, 'sendAll', '/summaryView', true, false), selected : true }
						}
					}
				}
			}, {
		...
		$function.switchMenuItem = function(obj, panel,menuType){
			...
			// 新建纪要
			function createSummary(mainModelName, modelName, type, path) {
				var url = $var.$contextPath + '/km/imissive/include/send/createDoc_' + type + '_relation.jsp?titleMsgKey=titleMsgKey.summary&titleBundle=km-imeeting';
				url = Com_SetUrlParameter(url, 'mainModelName', mainModelName);
				url = Com_SetUrlParameter(url, 'modelName', modelName);
				url = Com_SetUrlParameter(url, 'urlParam', 'fdModelName:com.landray.kmss.km.imeeting.model.KmImeetingMain@isRelationFile:1');
				url = Com_SetUrlParameter(url, 'isSimpleCategory', 'false');
				url = Com_SetUrlParameter(url, 'fdModelName', 'com.landray.kmss.km.imeeting.model.KmImeetingMain');
				url = Com_SetUrlParameter(url, 'j_path', path);
				LUI.pageOpen(url, '_rIframe');
			}
