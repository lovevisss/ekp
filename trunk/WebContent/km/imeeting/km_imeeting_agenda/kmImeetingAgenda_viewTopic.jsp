...
<script type="text/javascript">
    ...
    window.buildAttendUnits = function (ids) {
        if (!ids) {
            console.log("aids", ids);
            $(".attendUnitTable").hide();
            return;
        }
        var data = new KMSSData();
        var url = "${KMSS_Parameter_ContextPath}km/imeeting/km_imeeting_agenda/kmImeetingAgenda.do?method=loadAttendUnitList&ids=" + ids;
    ...
    <table class="tb_normal" width=100% id="TABLE_ViceDocList" align="center">
        ...
        <c:forEach items="${kmImeetingMainForm.kmImeetingVicePlaceDetailForms}" var="kmImeetingVicePlaceDetailitem" varStatus="vstatus">
            <tr KMSS_IsContentRow="1">
                ...
                <td align="center">
                    <input type="hidden" name="kmImeetingVicePlaceDetailForms[${vstatus.index}].fdMeetingId"
                           value="${kmImeetingVicePlaceDetailitem.fdMeetingId}"/>
                    <input type="hidden" name="kmImeetingVicePlaceDetailForms[${vstatus.index}].fdPlaceId"
                           value="${kmImeetingVicePlaceDetailitem.fdPlaceId }"/>
                        ...
                        <c:if test="${hasSysAttend == true }">
                            <c:import url="/sys/attend/import/map_tag.jsp" charEncoding="UTF-8">
                                <c:param name="propertyName"
                                         value="kmImeetingVicePlaceDetailForms[${vstatus.index}].fdOtherPlace" />
                                <c:param name="nameValue"
                                         value="${kmImeetingVicePlaceDetailitem.fdOtherPlace }" />
                                <c:param name="propertyCoordinate"
                                         value="kmImeetingVicePlaceDetailForms[${vstatus.index}].fdOtherPlaceCoordinate" />
                                <c:param name="coordinateValue"
                                         value="${kmImeetingVicePlaceDetailitem.fdOtherPlaceCoordinate }" />
                            </c:import>
                        </c:if>
            ...
            <td KMSS_IsContentRow="1" width="4%" class="index_num">
                ...
                <c:choose>
                    <c:when test="${isOnMainPage eq 'true' && not empty kmImeetingAgendaitem.fdAttKeyId}">
                        <input type="hidden" name="kmImeetingAgendaForms[${vstatus.index}].fdId"
                               value="${kmImeetingAgendaitem.fdAttKeyId}" id="detail_fdTopicId"/>
                    </c:when>
                    <c:otherwise>
                        <input type="hidden" name="kmImeetingAgendaForms[${vstatus.index}].fdId"
                               value="${kmImeetingAgendaitem.fdId}" id="detail_fdTopicId"/>
                    </c:otherwise>
                </c:choose>
                <i class="meeting-content-num">${vstatus.index+1}</i>
            </td>
            <td width="95%">
                <c:set value="${kmImeetingAgendaitem.docReporterName}" var="topicDocReporterName"/>
                <c:if test="${empty kmImeetingAgendaitem.docReporterName}">
                    <c:set value="<无>" var="topicDocReporterName"/>
                </c:if>
                <div class="meeting-content-item">
                    <div class="meeting-content-panel">
                        <div>
                            <div class="meeting-view-title clearfix">
                                <p class="meeting-view-title-name td_reporter" title="${topicDocReporterName}">
                                    ${topicDocReporterName}
                                </p>
                            ...
                            <div class="meeting-view-content">
                                <div class="meeting-view-content-top">
                                    <div>
                                        <p class="meeting-view-content-title td_subject"><c:out
                                                value="${kmImeetingAgendaitem.docSubject}"></c:out></p>
                                        <div class="meeting-view-content-line">
                                            <p class="meeting-view-content-desc">
                                                <bean:message bundle="km-imeeting" key="kmImeetingTopic.organizer"/>：</p>
                                        ...
                                        <div class="meeting-view-content-line">
                                            ...
                                            <p class="meeting-view-content-info td_respons">${kmImeetingAgendaitem.docResponsName}</p>
                                        </div>
                                        <div class="meeting-view-content-line">
                                        ...
                                        <div class="meeting-view-content-line">
                                            <p class="meeting-view-content-desc">
                                                <bean:message bundle="km-imeeting" key="kmImeetingTopic.pizhufujian"/>：
                                            </p>
                                            <span class="td_fromTopic">
                                                <input type="hidden" name="kmImeetingAgendaForms[${vstatus.index}].fdFromTopicId"
                                                       value="${kmImeetingAgendaitem.fdFromTopicId}" class="muiTopicId"/>
                                            ...
                                            <p class="meeting-view-content-info">
                                                <c:import url="/sys/attachment/sys_att_main/sysAttMain_view.jsp"
                                                          charEncoding="UTF-8">
                                                    <c:param name="fdKey"
                                                             value="issueAttBoen_${kmImeetingAgendaitem.fdId }"/>
                                                    <c:param name="fdModelName"
                                                             value="com.landray.kmss.km.imeeting.model.KmImeetingMain"/>
                                ...
                                <div class="meeting-view-content-bottom">
                                    <div class="meeting-view-content-line">
                                        <p class="meeting-view-content-desc">
                                            <bean:message bundle="km-imeeting" key="kmImeetingTopic.recommended.attendance.units"/>：
                                        </p>
                                        <ul class="meeting-view-content-list clearfix">
                                            <li class="td_attendUnit">${kmImeetingAgendaitem.fdAttendUnitNames}</li>
                                        </ul>
                                    </div>
                                    <div class="meeting-view-content-line">
                                        <p class="meeting-view-content-desc">
                                            <bean:message bundle="km-imeeting" key="kmImeetingTopic.suggested.listening.units"/>：
                                        </p>
                                        <ul class="meeting-view-content-list clearfix">
