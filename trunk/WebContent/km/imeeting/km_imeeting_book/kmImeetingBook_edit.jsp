...
<template:include ref="default.dialog">
	<template:replace name="head">
		<link rel="stylesheet" type="text/css" href="${LUI_ContextPath}/km/imeeting/resource/css/book.css">
		</link>
	</template:replace>
	<template:replace name="content">
		<html:form action="/km/imeeting/km_imeeting_book/kmImeetingBook.do" styleId="bookform">
			...
			<input type="hidden" name="updateType" value="${param.updateType}">
			<input type="hidden" name="updateDate" value="${param.updateDate}">
			<table class="tb_normal" width="98%" style="margin-top: 15px;">
				<tr>
					<td colspan="4" class="com_subject" style="font-weight: bold;">
						<bean:message bundle="km-imeeting" key="kmImeetingBook.baseInfo" />
					</td>
				</tr>
				<tr>
					<%--会议名称--%>
				...
				<tr>
					...
					<td width="85%" colspan="3">
						<xform:datetime property="fdHoldDate" dateTimeType="datetime" validators="after compareTime"
							required="true" onValueChange="changeHoldDate">
						</xform:datetime>
						<span style="position: relative; top: -5px;">~</span>
						<xform:datetime property="fdFinishDate" dateTimeType="datetime" validators="after compareTime"
							required="true">
						</xform:datetime>
						<input type="hidden" name="fdHoldDuration" />
						<div style="display: inline-block;">
							<c:if test="${param.updateType ne 'cur'}">
								<ui:recurrence id="fdRecurrence" property="fdRecurrenceStr"
				...
				<tr>
					<td colspan="4" class="com_subject" style="font-weight: bold;">
						<bean:message bundle="km-imeeting" key="kmImeetingBook.fdPlace.detail" />
					</td>
				</tr>
				<tr>
					<%--会议地点--%>
					...
					<td width="35%">
						<div style="display: none;">
							<input type="text" name="fdUserTime" value="${res.fdUserTime}"
								validate="validateUserTime" />
						</div>
						<c:if test="${empty res }">
							<xform:dialog propertyId="fdPlaceId" propertyName="fdPlaceName" showStatus="edit"
								validators="validateUserTime" className="inputsgl" style="width:95%;" required="true"
					...
					<td width="35%"><span id="resKeeperName">
							<c:if test="${not empty res.docKeeper}">
								<c:choose>
									...
									<c:otherwise>
										<c:out value="${res.docKeeper.fdName}"></c:out>
										<bean:message bundle="km-imeeting" key="kmImeetingRes.docKeeper.disable" />
									</c:otherwise>
								</c:choose>
							</c:if>
						</span></td>
				...
				<tr>
					...
					<td width="35%"><span id="resAddressFloor">
				...
				<tr>
					...
					<td width="85%" colspan="3"><span id="resDetail">
			...
			<div style="margin: 15px auto; width: 200px;">
				<center>
					...
					<c:if test="${kmImeetingBookForm.method_GET=='edit' }">
						<kmss:auth
							requestURL="/km/imeeting/km_imeeting_book/kmImeetingBook.do?method=delete&fdId=${kmImeetingBookForm.fdId}"
							requestMethod="GET">
							<ui:button text="${lfn:message('button.delete')}" styleClass="lui_toolbar_btn_gray"
								onclick="del();" />&nbsp;
						</kmss:auth>
					</c:if>
					<ui:button text="${lfn:message('button.close')}" styleClass="lui_toolbar_btn_gray"
...
<script>
	...
	seajs.use([
		...
		window.selectHoldPlace = function() {
			...
			var url = "/km/imeeting/km_imeeting_res/kmImeetingRes_showResDialog.jsp?fdHoldDate=" + fdHoldDate +
				"&fdFinishDate=" + fdFinishDate + "&resId=" + resId + "&resName=" + encodeURI(resName);
		...
		validation.addValidator('compareTime', '${lfn:message("km-imeeting:kmImeetingMain.fdDate.tip")}', function(
			v, e, o) {
		...
		validation.addValidator('validateUserTime',
			'${lfn:message("km-imeeting:kmImeetingRes.fdUserTime.error.tip")}',
			function(v, e, o) {
				...
				if (result == false) {
					...
					error = error.replace('%fdPlaceName%', $('[name="fdPlaceName"]').val()).replace(
						'%fdUserTime%', $('[name="fdUserTime"]').val());
		...
		window.del = function() {
			...
			var url = "${LUI_ContextPath}/km/imeeting/km_imeeting_book/kmImeetingBook.do?method=delete&fdId=" +
				fdId;
		...
		window.save = function() {
			...
			if (!_validateUserTime()) {
				var error = '${lfn:message("km-imeeting:kmImeetingRes.fdUserTime.error.tip")}';
				error = error.replace('%fdPlaceName%', $('[name="fdPlaceName"]').val()).replace('%fdUserTime%',
					$('[name="fdUserTime"]').val());
				dialog.alert(error);
				return;
			}
			...
			$.ajax({
				...
				success: function(data, textStatus, xhr) { //操作成功
					...
					if (data && !data.result) {
						...
						//冲突
						dialog.alert("${lfn:message('km-imeeting:kmImeetingBook.conflict.tip')}"
							.replace('%Place%', $('[name="fdPlaceName"]').val()));
					}
				}
			});
		};
		...
		var ajaxSubmit = function(method) {
			...
			$('[name="fdHoldDuration"]').val(((end.getTime() - start.getTime()) / 3600000).toFixed(
			2)); //记录会议历时
				...
				success: function(data, textStatus, xhr) { //操作成功
					if (data && data['status'] === true) {
						...
						if('${param.source}'==='reception'){
							localStorage.setItem('kmImeetingBook', $('[name="fdHoldDate"]').val() + "~ " + $('[name="fdFinishDate"]').val() +"，"+ $('[name="fdPlaceName"]').val()+"会议室");
						}
						if (window.$dialog != null) {
							$dialog.hide("success");
						} else {
