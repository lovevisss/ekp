...
<template:include ref="default.print" sidebar="no">
	...
	<template:replace name="content">
		...
		<form name="kmImeetingSummaryFormTemp" method="post" action="<c:url value="/km/imeeting/km_imeeting_summary/kmImeetingSummary.do" />">
		<center>
		...
		<center id="print_content">
			<c:forEach items="${kmImeetingSummaryFormList }" var="kmImeetingSummaryFormTemp" varStatus="vstatus">
				<kmss:auth requestURL="/km/imeeting/km_imeeting_Summary/kmImeetingSummary.do?method=view&fdId=${kmImeetingSummaryFormTemp.fdId}" requestMethod="GET">
					<c:set var="kmImeetingSummaryForm" value="${kmImeetingSummaryFormTemp }" scope="request" />
					<div class="print_title_header">
						<p id="title" class="print_txttitle">
							<bean:write name="kmImeetingSummaryForm" property="fdName" />
						</p>
						<div class="printDate">
								...
								<table name="info_content" width=100%>
									<tr>
										<td name="_xform_detail">
											<c:import url="/km/imeeting/km_imeeting_summary/import/kmImeetingSummary_viewBase.jsp">
											</c:import>
										</td>
									</tr>
								</table>
					...
					<% 
						...
						// 清除表单信息
						request.removeAttribute("kmImeetingSummaryForm"); 
					%>
				</kmss:auth>
			</c:forEach>
		</center>
