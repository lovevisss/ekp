...
<script type="text/javascript">
    ...
    seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog', 'lui/topic', 'lui/toolbar'], function ($, strutil, dialog, topic, toolbar) {
        ...
        window.expanded = function () {
            LUI('summaryCriteria').expandCriterions(!LUI('summaryCriteria').expand);
        };

        window.doSearchAction = function (evt) {
            var topicEvent = {
                criterions: LUI("summaryCriteria")._buildCriteriaSelectedValues(),
                query: []
            };
            var obj = {};
            var values = [];
        ...
        window.addDoc = function () {
            dialog.categoryForNewFile(
                ...
                '/km/imeeting/km_imeeting_summary/kmImeetingSummary.do?method=add&fdTemplateId=!{id}', false, null, null, cateId);
        };
        ...
        window.batchPrint = function () {
            ...
            var url = '<c:url value="/km/imeeting/km_imeeting_summary/kmImeetingSummary.do?method=printBatch"/>' + '&fdIds=' + values;
            Com_OpenWindow(url);
        };
