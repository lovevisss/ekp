...
<%
	...
	pageContext.setAttribute("wpsoaassistEmbed", new Boolean(SysAttWpsoaassistUtil.isWPSOAassistEmbed(request)));
%>
	...
	<c:if test="${param.approveModel ne 'right'}">
		...
		<div class="lui_form_content_frame" style="padding-top:20px">
			...
			<c:if test="${kmImeetingSummaryForm.method_GET eq 'edit' && pageScope._isWpsCenterEnable eq 'true'}">
			...
			<table class="tb_normal" width=100% id="Table_Main">
				...
				<tr>
					...
					<td width="85%" colspan="3">
						<xform:address style="width:45%;" propertyId="fdHostId" propertyName="fdHostName" orgType="ORG_TYPE_PERSON"></xform:address>
					</td>
				</tr>
				<tr>
					<%-- 会议时间--%>
					...
					<td width="35%">
						<xform:datetime property="fdHoldDate" dateTimeType="datetime" onValueChange="changeDateTime"
							 required="true" validators="compareTime"></xform:datetime>
				...
				<tr>
					...
					<td width=85% colspan="3">
						<c:if test="${kmImeetingSummaryForm.fdMeetingId == null}">
							 <xform:address mulSelect="true" isLoadDataDict="false" required="false" style="width:97%;height:80px" textarea="true"
				...
				<tr>
					...
					<td width=85% colspan="3">
						<c:if test="${kmImeetingSummaryForm.fdMeetingId == null}">
							<xform:address mulSelect="true" isLoadDataDict="false" required="false" style="width:97%;height:80px" textarea="true"
							 propertyId="fdPlanInvitePersonIds" subject="${lfn:message('km-imeeting:kmImeetingSummary.fdPlanInvitePersons')}" 
							 propertyName="fdPlanInvitePersonNames" orgType='ORG_TYPE_ALL' onValueChange="changeActualAttendPerson"></xform:address>
						</c:if>
						<c:if test="${kmImeetingSummaryForm.fdMeetingId != null}">
							<html:hidden property="fdPlanInvitePersonIds" />
							<html:hidden property="fdPlanInvitePersonNames" />
				...
				<tr>
					...
					<td colspan="3">
						<xform:address mulSelect="true" isLoadDataDict="false" required="true" style="width:97%;height:80px" textarea="true"
							 propertyId="fdActualAttendPersonIds" subject="${lfn:message('km-imeeting:kmImeetingSummary.fdActualAttendPersons')}" 
							 propertyName="fdActualAttendPersonNames" orgType='ORG_TYPE_ALL' />
					</td>
				</tr>
				<c:if test="${not (kmImeetingSummaryForm.method_GET eq 'edit' && pageScope._isWpsCenterEnable eq 'true')}">
					<tr>
						<%-- 编辑方式--%>
						<td class="td_normal_title" width=15%>
					...
					<td width=85% colspan="3">
						...
						<c:choose>
							<c:when test="${pageScope._isWpsWebCloudEnable == 'true' or pageScope._isWpsWebOfficeEnable == 'true' or pageScope._isWpsCenterEnable == 'true'}" >
								<div id="wordEdit" <c:if test="${kmImeetingSummaryForm.fdContentType=='rtf'}">style="display:none;width:0px;height:0px;"</c:if>>
							</c:when>
							<c:otherwise>
							...
							<c:choose>
								...
								<c:when test="${pageScope._isWpsCenterEnable == 'true'}">
									<c:if test="${(kmImeetingSummaryForm.method_GET == 'add') || (kmImeetingSummaryForm.method_GET == 'edit' && kmImeetingSummaryForm.fdContentType != 'rtf')}">
...
<script>
	seajs.use(['lui/jquery','lui/dialog','lui/topic','km/imeeting/resource/js/dateUtil'], function($,dialog,topic,dateUtil){
		...
		window.changeActualAttendPerson=function(){
			...
			inviteIds=$('[name="fdPlanInvitePersonIds"]').val(),
			inviteNames=$('[name="fdPlanInvitePersonNames"]').val();
			participantIds=$('[name="fdPlanParticipantPersonIds"]').val(),
			participantNames=$('[name="fdPlanParticipantPersonNames"]').val();
			var actualAttendIds=joinPerson(attendIds,participantIds,inviteIds),
				actualAttendNames=joinPerson(attendNames,participantNames,inviteNames);
		...
		window.checkEditType=function(value, obj){
			...
			if("word" == value){
				_wordEdit.css({'display':"block",'width':"100%",'height':"650px"});
				wordFloat.css({'width':'auto','height':'600px','filter':'alpha(opacity=100)','opacity':'1'});
				_rtfEdit.css({'display':"none"});
				$('.attopenLocal').show();
				if ("${pageScope._isWpsWebCloudEnable}" == "true") {
					setTimeout(function(){
						wps_cloud_editonline.load();
						$("#WPSCloudOffice_editonline").height("620px");
					}, 1000);
				} else if ("${pageScope._isWpsWebOfficeEnable}" == "true"){
					wps_editonline.load();
				} else if ("${pageScope._isWpsCenterEnable}" == "true") {
					_wordEdit.css({'display':"block",'width':"100%",'height':"721px"});
					wps_center_editonline.load();
					$("#WPSCenterOffice_editonline").css({"width":"100%"});
		...
		window.commitMethod=function(commitType, saveDraft) {
			...
			var fdContentTypeVal = $("input[name='fdContentType']:checked").val();
			if ("rtf" == fdContentTypeVal) {
				$("#wordEdit").css({'display':"block",'width':"0px",'height':"0px"});
				$('.attopenLocal').hide();
				$("#wordEditFloat").css({'width':'0px','height':'0px','filter':'alpha(opacity=0)','opacity':'0'});
				$("#JGWebOffice_editonline").height("0px");
				chromeHideJG_2015(0);
			}
			if(saveDraft=="true"){
