                    ...
                    <ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" count="6">
                        <list:sortgroup>
                            <list:sort property="docCreateTime"
                                       text="${lfn:message('km-imeeting:kmImeetingSummary.docCreateTime') }"
                                       group="sort.list" value="down"></list:sort>
                            <list:sort property="docPublishTime"
                                       text="${lfn:message('km-imeeting:kmImeetingSummary.docPublishTime') }"
                    ...
                    <ui:toolbar count="4" id="Btntoolbar">
                        <c:if test="${JsParam.categoryId==null || JsParam.nodeType=='CATEGORY'}">
                            <kmss:authShow roles="ROLE_KMIMEETING_SUMMARY_CREATE">
                                <ui:button title="${lfn:message('button.add')}" text="${lfn:message('button.add')}"
                                           onclick="addDoc()" order="2"></ui:button>
                            </kmss:authShow>
                        </c:if>
                        <c:if test="${JsParam.nodeType=='TEMPLATE'}">
                            <kmss:auth
                                    ...
                                    requestMethod="GET">
                                <ui:button title="${lfn:message('button.add')}" text="${lfn:message('button.add')}"
                                           onclick="Com_OpenWindow('${LUI_ContextPath }/km/imeeting/km_imeeting_summary/kmImeetingSummary.do?method=add&fdTemplateId=${JsParam.categoryId}&fdProjectId=${JsParam.fdProjectId}&fdModelName=${JsParam.fdModelName}');"
                                           order="2"></ui:button>
                            </kmss:auth>
                        </c:if>
                        <kmss:auth
                        ...
                        <kmss:auth
                            ...
                            <ui:button id="chgCate" text="${lfn:message('km-imeeting:kmImeeting.btn.translate')}"
                                       order="4" onclick="chgSelect();"></ui:button>
                        </kmss:auth>
                        <%-- 收藏 --%>
            ...
            seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog', 'lui/topic', 'lui/toolbar'], function ($, strutil, dialog, topic, toolbar) {
                ...
                window.addDoc = function () {
                    dialog.categoryForNewFile(
                        ...
                        '/km/imeeting/km_imeeting_summary/kmImeetingSummary.do?method=add&fdTemplateId=!{id}', false, null, null, docCategoryId);
                };
                //删除
