			...
			<td>
				<c:choose>
					<c:when test="${not empty kmImeetingMainForm.fdVicePlaceNames or not empty kmImeetingMainForm.fdOtherVicePlace }">
						...
						<c:if test="${hasSysAttend == true }">
							<c:import url="/sys/attend/import/map_tag.jsp" charEncoding="UTF-8">
								...
								<c:param name="style" value="width:90%;display:block;"></c:param>
						...
						<c:if test="${hasSysAttend == false }">
							<xform:text property="fdOtherVicePlace" style="width:95%;display:block;"></xform:text>
						</c:if>
					...
					<c:otherwise>
						...
						<c:if test="${hasSysAttend == false }">
							<xform:text property="fdOtherPlace" style="width:95%;display:block;"></xform:text>
						</c:if>
					</c:otherwise>
				</c:choose>
			</td>
