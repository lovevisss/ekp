	...
	<div class="muiImeetingInfoCard">
		...
		<c:if test="${not empty __kmImeetingMainForm.fdHoldDate or not empty __kmImeetingMainForm.fdFinishDate }">
			<div class="muiImeetingInfoItem" style="display: flex;">
				...
				<span class="meeting-date" style="white-space: pre-wrap;"><span id="meetingHoldDate"><xform:datetime property="fdHoldDate" dateTimeType="datetime"></xform:datetime></span> - <span id="meetingFinishDate"><xform:datetime property="fdFinishDate" dateTimeType="datetime"></xform:datetime></span></span>
			</div>
		</c:if>
				...
				<c:forEach items="${seatNumbers }" var="seatNumber" varStatus="varStatus">
					<c:choose>
						<c:when test="${varStatus.index == 0}">
							<span><c:out value="${seatNumber}"/><br/></span>
						</c:when>
						<c:otherwise>
							<span style="margin-left: 56px;"><c:out value="${seatNumber}"/><br/></span>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</div>
