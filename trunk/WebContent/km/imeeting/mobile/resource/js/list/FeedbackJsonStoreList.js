define([
	...
	return declare("km.imeeting.list.FeedbackJsonStoreList", [JsonStoreList], {
		...
		startup:function(){
			this.inherited(arguments);
			this.subscribe('/mui/property/filter', 'onFeedbackCriteria');
		},
		
		//回执单筛选
		onFeedbackCriteria:function(widget, criValObj) {
			// 是否显示回执状态标签
			if (criValObj.criteriaType) {
				window.isShowFeedbackStatusTag = false;
			} else {
				window.isShowFeedbackStatusTag = true;
			}
			criteriaType = criValObj.criteriaType || "";
