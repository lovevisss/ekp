...
//						detailStr += '<li><i class="mui mui-people"></i>' + (occupiedDetail.fdHost || '') + '</li>';
						...
						if(occupiedDetail.fdHasExam == 'wait') {
							self.placeNameNode.innerHTML= '会议室审核中';
							self.fdPhone = occupiedDetail.fdPlaceKeeperPhone;
							domAttr.set(self.infoMainNode, 'data-flag', 'callKeeper');
							self.operationTextNode.innerHTML = '保管员';
						} else {
							self.placeNameNode.innerHTML= '会议室被占用';
							self.fdPhone = occupiedDetail.fdPhone;
							domAttr.set(self.infoMainNode, 'data-flag', 'callHost');
							self.operationTextNode.innerHTML = '预约人';
						}
						var detailStr = '<li>内容：' + (occupiedDetail.fdName || '') + '</li>';
						detailStr += '<li>预约人：' + (occupiedDetail.fdHost || '') + '</li>';
						detailStr += '<li>楼层：' + (srcObj.fdAddressFloor || '') + '</li>';
		...
		showPlaceDetail:function(srcObj){
			...
			this.operationTextNode.innerHTML = '下一步';
...
//			detailStr += '<li><i class="mui mui-asset"></i>' + (srcObj.fdDetail || '') +  '</li>';
			...
			this.placeNameNode.innerHTML= '会议室空闲';
			var detailStr = '<li>会议室：' + srcObj.fdName + '</li>';
			detailStr += '<li>地点：' + (srcObj.fdAddressFloor || '') +  '</li>';
			detailStr += '<li>设备：' + (srcObj.fdDetail || '') +  '</li>';
				...
				if(this.createType){
					...
					if(this.createType == 'meeting'){
						...
						if(this.canCreateCloud) {
							...
							Tip.tip({icon:'mui mui-warn', text:'很抱歉，您没有新建会议权限！',width:'260',height:'120'});
						}
					}
					if(this.createType == 'book'){
						if(this.canCreateBook) {
							...
							Tip.tip({icon:'mui mui-warn', text:'很抱歉，您没有预约会议权限！',width:'260',height:'120'});
						}
					}
				}else{
				...
				if(evt.detail){
					if(this.start.length == 0 && this.finish.length == 0){
						...
						if(srcObj.fdId == this.placeId){
							this.showStatusArea();
							Tip.tip({icon:'mui mui-warn', text:'所选时间时间包含非空闲时间！！',width:'260',height:'120'});
						}else{
							this.showPlaceOccupiedDetail(srcObj,evt.detail,evt.value);
						}
					}
