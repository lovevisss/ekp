define([
	...
	var item = declare("km.imeeting.list.item.CalendarItemMixin", [ItemBase, _ListLinkItemMixin], {
		...
		buildRendering:function(){
			...
			//标题
			domConstruct.create("div", { className: 'meeting_timeline_subject',innerHTML:util.formatText(this.title) }, content);
			...
			if(this.fdPlaceName) {				
				if(this.fdVicePlacesNames || this.fdOtherVicePlace) {
					...
					domConstruct.create("span", {innerHTML: msg['kmImeeting.fdMainPlace'] + '：' + util.formatText(this.fdPlaceName)}, placeNode);
					...
					domConstruct.create("p", {className:'item_other_place',innerHTML: msg['kmImeeting.fdVicePlace'] + '：' + util.formatText(t.join(' '))}, otherPlaceNode);
				} else {
