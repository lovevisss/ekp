...
<list:data>
	<list:data-columns var="kmImeetingMain" list="${queryPage.list }" varIndex="status" mobile="true">
		...
		<%-- 会议名称--%>
		<list:data-column col="fdName" title="${ lfn:message('km-imeeting:kmImeetingMain.fdName') }" escape="false">
			<c:choose>
				<c:when test="${not empty kmImeetingMain.fdName}">
					<c:out value="${kmImeetingMain.fdName}"/>
				</c:when>
				<c:otherwise>
					<c:out value="${kmImeetingMain.docSubject}"/>
				</c:otherwise>
			</c:choose>
		</list:data-column>
		...
		<%-- 发布时间--%>
      	<list:data-column col="docPublishTime" title="发布时间">
	        <kmss:showDate value="${kmImeetingMain.docPublishTime}" type="datetime"></kmss:showDate>
      	</list:data-column>
      	
      	<%-- 创建时间--%>
      	<list:data-column col="created" title="创建时间">
	        <kmss:showDate value="${kmImeetingMain.docCreateTime}" type="datetime"></kmss:showDate>
      	</list:data-column>
		...
		<%-- 召开时间--%>
		<list:data-column col="holdTime" escape="false">
			<c:if test="${not empty kmImeetingMain.fdHoldDate }">
				<kmss:showDate value="${kmImeetingMain.fdHoldDate}" type="datetime"></kmss:showDate>
			</c:if>
		</list:data-column>
		
		<%-- 结束时间--%>
		<list:data-column col="endTime" escape="false">
			<c:if test="${not empty kmImeetingMain.fdFinishDate }">
				<kmss:showDate value="${kmImeetingMain.fdFinishDate}" type="datetime"></kmss:showDate>
			</c:if>
		</list:data-column>
		
		<list:data-column col="creator" title="创建人">
			<c:out value="${kmImeetingMain.docCreator.fdName}"></c:out>
      	</list:data-column>
		...
		<%-- <list:data-column col="host" title="${ lfn:message('km-imeeting:kmImeetingMain.fdHost') }" escape="false">
	 	...
	 	<%-- <list:data-column col="place" title="${ lfn:message('km-imeeting:kmImeetingMain.fdPlace') }" escape="false">
	       ...
	       <%
		       ...
		       if(pageContext.getAttribute("kmImeetingMain")!=null){
		    	   ...
		    	   if("00".equals(docStatus )){
		    		   ...
		    		   request.setAttribute("meetingStatus", "00");
		    	   }
		    	   if("10".equals(docStatus )){
		    		   request.setAttribute("statusText", ResourceUtil.getString("status.draft"));
		    		   request.setAttribute("meetingStatus", "10");
		    	   }
		    	   if("11".equals(docStatus )){
		    		   request.setAttribute("statusText", ResourceUtil.getString("km-imeeting:kmImeeting.status.refuse"));
		    		   request.setAttribute("meetingStatus", "11");
		    	   }
		    	   if("20".equals(docStatus )){
		    		   request.setAttribute("statusText", ResourceUtil.getString("status.examine"));
		    		   request.setAttribute("meetingStatus", "20");
		    	   }
		    	   if("30".equals(docStatus) && !isBegin){
		    		   ...
		    		   request.setAttribute("meetingStatus", "unHold");
		    	   }
		    	   if("30".equals(docStatus) && isBegin && !isEnd){
		    		   ...
		    		   request.setAttribute("meetingStatus", "holding");
		    	   }
      	...
      	<%-- 会议状态 --%>
		<list:data-column col="meetingStatus" escape="false">
			${meetingStatus}
		</list:data-column>
        ...
        <list:data-column col="docStatus" title="${lfn:message('km-supervise:kmImeetingMain.docStatus')}">
			<c:choose>
				...
				<c:when test="${kmImeetingMain.docStatus=='11' }">
					<bean:message bundle="km-imeeting" key="kmImeeting.status.refuse"></bean:message>
				</c:when>
				<c:when test="${kmImeetingMain.docStatus=='30' || kmImeetingMain.docStatus=='40' }">
					<bean:message bundle="km-imeeting" key="kmImeeting.status.publish"></bean:message>
				</c:when>
		...
		<list:data-column col="tagType" escape="false">
			<c:choose>
				<c:when test="${docType eq 'coming'}">
					<c:out value="coming"></c:out>
				</c:when>
				<c:when test="${docType eq 'myJoined'}">
					<c:out value="joined"></c:out>
				</c:when>
				<c:when test="${docType eq 'myDraft'}">
					<c:out value="drafed"></c:out>
				</c:when>
			</c:choose>
		</list:data-column>
		...
		<%-- 当前节点--%>
	 	<list:data-column col="summary" title="当前节点" escape="false">
			<kmss:showWfPropertyValues idValue="${kmImeetingMain.fdId}" propertyName="summary" />
		</list:data-column>
	</list:data-columns>
