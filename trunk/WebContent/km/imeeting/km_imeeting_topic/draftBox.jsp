...
<link type="text/css" rel="stylesheet" href="${LUI_ContextPath}/sys/profile/resource/css/operations.css?s_cache=${LUI_Cache}"/>
	...
	<list:listview id="listview_topic"   >
		<ui:source type="AjaxJson">
				{url:'/km/imeeting/km_imeeting_topic/kmImeetingTopic.do?method=list&q.docStatus=10'}
		</ui:source>
 	...
 	<script type="text/javascript">
		...
		seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
			...
			window.delDoc = function(){
				...
				/* 软删除配置 */
				var url='<c:url value="/km/imeeting/km_imeeting_topic/kmImeetingTopic.do?method=deleteall&draft=true"/>';
				var config = {
					url : url, // 删除数据的URL
			...
			window.deleteDoc = function(fdId){
				...
				var config = {
					url : url, // 删除数据的URL
					data : $.param({"List_Selected":values},true), // 要删除的数据
					modelName : "com.landray.kmss.km.imeeting.model.KmimeetingTopic" // 主要是判断此文档是否有部署软删除
				};
				// 通用删除方法
				Com_Delete(config, delCallback);
			};
