...
<template:replace name="title">
	<c:choose>
		<c:when test="${ kmImeetingTopicForm.method_GET == 'add' }">
			<c:out value="${lfn:message('km-imeeting:kmImeetingTopic.addDoc') } - ${ lfn:message('km-imeeting:module.km.imeeting') }"></c:out>	
		</c:when>
		<c:otherwise>
  	...
  	<c:choose>
		...
		<c:when test="${ kmImeetingTopicForm.method_GET == 'edit' }">
			...
			<c:choose>
				...
				<c:otherwise>
					<c:if test="${kmImeetingTopicForm.docStatusFirstDigit<'3'}">
						<c:if test="${isShowBtn eq 'true' && param.method ne 'editIncorporate'}">
